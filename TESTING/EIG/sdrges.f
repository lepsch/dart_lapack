         ...
         for (JTYPE = 1; JTYPE <= MTYPES; JTYPE++) { // 180
            ...
            if ( KCLASS( JTYPE ).LT.3 ) {
               ...
               if ( KCLASS( JTYPE ).EQ.2 .AND. N.GT.0 ) {
                  ...
                  CALL SORM2R( 'L', 'N', N, N, N-1, Q, LDQ, WORK, A, LDA, WORK( 2*N+1 ), IINFO )                   IF( IINFO.NE.0 ) GO TO 100;
                  CALL SORM2R( 'R', 'T', N, N, N-1, Z, LDQ, WORK( N+1 ), A, LDA, WORK( 2*N+1 ), IINFO )                   IF( IINFO.NE.0 ) GO TO 100;
                  CALL SORM2R( 'L', 'N', N, N, N-1, Q, LDQ, WORK, B, LDA, WORK( 2*N+1 ), IINFO )                   IF( IINFO.NE.0 ) GO TO 100;
                  CALL SORM2R( 'R', 'T', N, N, N-1, Z, LDQ, WORK( N+1 ), B, LDA, WORK( 2*N+1 ), IINFO )                   IF( IINFO.NE.0 ) GO TO 100
               }
            } else {
            ...
            for (ISORT = 0; ISORT <= 1; ISORT++) { // 150
               ...
               if ( ISORT.GE.1 ) {
                  ...
                  KNTEIG = 0
                  for (I = 1; I <= N; I++) { // 140
                     if ( SLCTES( ALPHAR( I ), ALPHAI( I ), BETA( I ) ) .OR. SLCTES( ALPHAR( I ), -ALPHAI( I ), BETA( I ) ) ) {
                        KNTEIG = KNTEIG + 1
                     }
                     if ( I.LT.N ) {
                        if ( ( SLCTES( ALPHAR( I+1 ), ALPHAI( I+1 ), BETA( I+1 ) ) .OR. SLCTES( ALPHAR( I+1 ), -ALPHAI( I+1 ), BETA( I+1 ) ) ) .AND. ( .NOT.( SLCTES( ALPHAR( I ), ALPHAI( I ), BETA( I ) ) .OR. SLCTES( ALPHAR( I ), -ALPHAI( I ), BETA( I ) ) ) ) .AND. IINFO.NE.N+2 ) {
                           RESULT( 12 ) = ULPINV
                        }
                     }
                  } // 140
