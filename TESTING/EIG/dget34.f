      void dget34(RMAX, LMAX, NINFO, KNT ) {
         ...
         for (IAM = 1; IAM <= 2; IAM++) { // 30
            for (IB = 1; IB <= 9; IB++) { // 20
               for (IC = 1; IC <= 9; IC++) { // 10
                  ...
                  dcopy(4, VAL( 3 ), 0, Q, 5 );
                  dlaexc( true , 2, T, 4, Q, 4, 1, 1, 1, WORK, INFO )                   IF( INFO != 0 ) NINFO( INFO ) = NINFO( INFO ) + 1;
                  ...
                  for (IC12 = 2; IC12 <= 5; IC12++) { // 70
                     for (IC21 = 2; IC21 <= 4; IC21++) { // 60
                        for (IC22 = -1; 2 < 0 ? IC22 >= 1 : IC22 <= 1; IC22 += 2) { // 50
                           ...
                           dcopy(16, VAL( 1 ), 0, Q, 1 );
                           dcopy(4, VAL( 3 ), 0, Q, 5 );
                           dlaexc( true , 3, T, 4, Q, 4, 1, 1, 2, WORK, INFO )                            IF( INFO != 0 ) NINFO( INFO ) = NINFO( INFO ) + 1;
                  ...
                  for (ICM = 1; ICM <= 2; ICM++) { // 140
                     for (IB = 1; IB <= 5; IB++) { // 130
                        for (IC = 1; IC <= 5; IC++) { // 120
                           ...
                           dcopy(16, VAL( 1 ), 0, Q, 1 );
                           dcopy(4, VAL( 3 ), 0, Q, 5 );
                           dlaexc( true , 3, T, 4, Q, 4, 1, 2, 1, WORK, INFO )                            IF( INFO != 0 ) NINFO( INFO ) = NINFO( INFO ) + 1;
                           dhst01(3, 1, 3, T1, 4, T, 4, Q, 4, WORK, LWORK, RESULT );
                           RES = RESULT( 1 ) + RESULT( 2 );
                           if ( INFO == 0 ) {
                           ...
                           for (IC21 = 3; IC21 <= 4; IC21++) { // 230
                              for (IC22 = -1; 2 < 0 ? IC22 >= 1 : IC22 <= 1; IC22 += 2) { // 220
                                 for (ICM = 5; ICM <= 7; ICM++) { // 210
                                    ...
                                    dcopy(16, VAL( 1 ), 0, Q, 1 );
                                    dcopy(4, VAL( 3 ), 0, Q, 5 );
                                    dlaexc( true , 4, T, 4, Q, 4, 1, 2, 2, WORK, INFO )                                     IF( INFO != 0 ) NINFO( INFO ) = NINFO( INFO ) + 1;
