      void cqlt01(M, N, A, AF, Q, L, LDA, TAU, WORK, LWORK, RWORK, RESULT ) {
...
// =====================================================================
      ...
      if ( M >= N ) {
         if (N < M && N > 0) CALL CLACPY( 'Full', M-N, N, AF, LDA, Q( 1, M-N+1 ), LDA );
         IF( N > 1 ) CALL CLACPY( 'Upper', N-1, N-1, AF( M-N+1, 2 ), LDA, Q( M-N+1, M-N+2 ), LDA );
      } else {
      ...
      if ( M >= N ) {
         ...
         if (N > M && M > 0) CALL CLACPY( 'Full', M, N-M, AF, LDA, L, LDA );
         IF( M > 0 ) CALL CLACPY( 'Lower', M, M, AF( 1, N-M+1 ), LDA, L( 1, N-M+1 ), LDA );
      }
