...
*                       Test SGERQF
*
                        CALL SRQT01( M, N, A, AF, AQ, AR, LDA, TAU, WORK, LWORK, RWORK, RESULT( 1 ) )
                     ELSE IF( M.LE.N ) THEN
*
*                       Test SORGRQ, using factorization
*                       returned by SRQT01
*
                        CALL SRQT02( M, N, K, A, AF, AQ, AR, LDA, TAU, WORK, LWORK, RWORK, RESULT( 1 ) )
                     END IF
                     IF( M.GE.K ) THEN
*
...
*                       by SRQT01
*
                        CALL SRQT03( M, N, K, AF, AC, AR, AQ, LDA, TAU, WORK, LWORK, RWORK, RESULT( 3 ) )
                        NT = NT + 4
*
...
*
                           CALL SLACPY( 'Full', M, NRHS, B, LDA, X( N-M+1 ), LDA )
                           SRNAMT = 'SGERQS'
                           CALL SGERQS( M, N, NRHS, AF, LDA, TAU, X, LDA, WORK, LWORK, INFO )
*
*                          Check error code from SGERQS.
*
                           IF( INFO.NE.0 ) CALL ALAERH( PATH, 'SGERQS', INFO, 0, ' ', M, N, NRHS, -1, NB, IMAT, NFAIL, NERRS, NOUT )
*
