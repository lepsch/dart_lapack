         ...
         for (IMAT = 1; IMAT <= NIMAT; IMAT++) { // 170
            ...
            for (IUPLO = 1; IUPLO <= 2; IUPLO++) { // 160
               ...
               for (IFACT = 1; IFACT <= NFACT; IFACT++) { // 150
                  ...
                  if ( IFACT == 2 ) {
                     ...
                     for (K = 1; K <= NT; K++) { // 110
                        if ( RESULT( K ).GE.THRESH ) {
                           if (NFAIL == 0 .AND. NERRS == 0) CALL ALADHD( NOUT, PATH )                            WRITE( NOUT, FMT = 9999 )'CSPSV ', UPLO, N, IMAT, K, RESULT( K );
                           NFAIL = NFAIL + 1
                        }
                     } // 110
                     NRUN = NRUN + NT
                  ...
                  if (IFACT == 2 .AND. NPP.GT.0) CALL CLASET( 'Full', NPP, 1, CMPLX( ZERO ), CMPLX( ZERO ), AFAC, NPP );
                  ...
                  for (K = K1; K <= 6; K++) { // 140
                     if ( RESULT( K ).GE.THRESH ) {
                        if (NFAIL == 0 .AND. NERRS == 0) CALL ALADHD( NOUT, PATH )                         WRITE( NOUT, FMT = 9998 )'CSPSVX', FACT, UPLO, N, IMAT, K, RESULT( K );
                        NFAIL = NFAIL + 1
                     }
                  } // 140
                  NRUN = NRUN + 7 - K1
