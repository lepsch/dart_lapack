   ...
   90                   CONTINUE
               ...
               DO 150 INB = 1, NNB
                 ...
                 t // he block structure of D. AINV is a work array for
                  ...
                  SRNAMT = 'CSYTRF_AA'
                  LWORK = MAX( 1, N*NB + N )
                  CALL CSYTRF_AA( UPLO, N, AFAC, LDA, IWORK, AINV, LWORK, INFO )
...
c  100                CONTINUE
                  ...
                  IF( INFO.NE.K ) THEN
                     CALL ALAERH( PATH, 'CSYTRF_AA', INFO, K, UPLO, N, N, -1, -1, NB, IMAT, NFAIL, NERRS, NOUT )
                  END IF
...
*+    TEST 2 (Using TRS)
                  // Solve and compute residual for  A * X = B.
                     ...
                     SRNAMT = 'CSYTRS_AA'
                     LWORK = MAX( 1, 3*N-2 )
                     CALL CSYTRS_AA( UPLO, N, NRHS, AFAC, LDA, IWORK, X, LDA, WORK, LWORK, INFO )

                     // Check error code from CSYTRS and handle error.

                     IF( INFO.NE.0 ) THEN
                        IF( IZERO.EQ.0 ) THEN
                           CALL ALAERH( PATH, 'CSYTRS_AA', INFO, 0, UPLO, N, N, -1, -1, NRHS, IMAT, NFAIL, NERRS, NOUT )
                        END IF
                     } else {
