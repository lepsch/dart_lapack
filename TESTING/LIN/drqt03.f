...
*  =====================================================================
      ...
      IF( K.GT.0 .AND. N.GT.K ) CALL DLACPY( 'Full', K, N-K, AF( M-K+1, 1 ), LDA, Q( N-K+1, 1 ), LDA )       IF( K.GT.1 ) CALL DLACPY( 'Lower', K-1, K-1, AF( M-K+2, N-K+1 ), LDA, Q( N-K+2, N-K+1 ), LDA )
   ...
   10    CONTINUE
         ...
         for (ITRANS = 1; ITRANS <= 2; ITRANS++) { // 20
            ...
            SRNAMT = 'DORMRQ'
            IF( K.GT.0 ) CALL DORMRQ( SIDE, TRANS, MC, NC, K, AF( M-K+1, 1 ), LDA, TAU( MINMN-K+1 ), CC, LDA, WORK, LWORK, INFO )
