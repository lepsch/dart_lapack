...
*+    TEST 7
               ...
               for (ITRAN = 1; ITRAN <= 2; ITRAN++) { // 50
                  ...
                  if ( RESULT( 7 ).GE.THRESH ) {
                     if (NFAIL == 0 .AND. NERRS == 0) CALL ALAHD( NOUT, PATH )                      WRITE( NOUT, FMT = 9997 ) 'DTPCON', NORM, UPLO, DIAG, N, IMAT, 7, RESULT( 7 );
                     NFAIL = NFAIL + 1
                  }
                  NRUN = NRUN + 1
               } // 50
...
*+    TEST 9
                  ...
                  if ( RESULT( 8 ).GE.THRESH ) {
                     if (NFAIL == 0 .AND. NERRS == 0) CALL ALAHD( NOUT, PATH )                      WRITE( NOUT, FMT = 9996 )'DLATPS', UPLO, TRANS, DIAG, 'N', N, IMAT, 8, RESULT( 8 );
                     NFAIL = NFAIL + 1
                  }
                  if ( RESULT( 9 ).GE.THRESH ) {
                     if (NFAIL == 0 .AND. NERRS == 0) CALL ALAHD( NOUT, PATH )                      WRITE( NOUT, FMT = 9996 )'DLATPS', UPLO, TRANS, DIAG, 'Y', N, IMAT, 9, RESULT( 9 );
                     NFAIL = NFAIL + 1
                  }
                  NRUN = NRUN + 2
               } // 80
