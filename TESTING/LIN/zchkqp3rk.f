...
*  =====================================================================
      ...
      const              ONE = 1.0D+0, ZERO = 0.0D+0, CZERO = ( 0.0D+0, 0.0D+0 ), CONE = ( 1.0D+0, 0.0D+0 ), BIGNUM = 1.0D+38 ;
      // ..
      // .. Local Scalars ..
      String             DIST, TYPE;
      String             PATH;
         ...
         DO IN = 1, NN
            ...
            DO INS = 1, NNS
               NRHS = NSVAL( INS )
                  ...
                  CALL ZLATB4( PATH, 14, M, NRHS, TYPE, KL, KU, ANORM, MODE, CNDNUM, DIST )

                  SRNAMT = 'ZLATMS'
                  CALL ZLATMS( M, NRHS, DIST, ISEED, TYPE, S, MODE, CNDNUM, ANORM, KL, KU, 'No packing', COPYB, LDA, WORK, INFO )

                  // Check error code from ZLATMS.

                  if ( INFO.NE.0 ) {
                     CALL ALAERH( PATH, 'ZLATMS', INFO, 0, ' ', M, NRHS, -1, -1, -1, 6, NFAIL, NERRS, NOUT )
                     CYCLE
                  }
            ...
            // 19. Random, CNDNUM = 2, scaled near overflow                            CNDNUM = 2                      LARGE = 1.0/( 0.25 * ( SAFMIN / EPS ) )  3 ( geometric distribution of singular values )
               ...
               if ( IMAT.EQ.1 ) {
                  ...
                  SRNAMT = 'ZLATMS'
                  CALL ZLATMS( M, N, DIST, ISEED, TYPE, S, MODE, CNDNUM, ANORM, KL, KU, 'No packing', COPYA, LDA, WORK, INFO )
                  ...
                  CALL ZLASET( 'Full', M, NB_ZERO, CZERO, CZERO, COPYA, LDA )

                     // 2) Generate an M-by-(N-NB_ZERO) matrix with the
                        // chosen singular value distribution
                        // in COPYA(1:M,NB_ZERO+1:N).

                  CALL ZLATB4( PATH, IMAT, M, NB_GEN, TYPE, KL, KU, ANORM, MODE, CNDNUM, DIST )

                  SRNAMT = 'ZLATMS'

                  IND_OFFSET_GEN = NB_ZERO * LDA

                  CALL ZLATMS( M, NB_GEN, DIST, ISEED, TYPE, S, MODE, CNDNUM, ANORM, KL, KU, 'No packing', COPYA( IND_OFFSET_GEN + 1 ), LDA, WORK, INFO )

                  // Check error code from ZLATMS.

                  if ( INFO.NE.0 ) {
                     CALL ALAERH( PATH, 'ZLATMS', INFO, 0, ' ', M, NB_GEN, -1, -1, -1, IMAT, NFAIL, NERRS, NOUT )
                     CYCLE
                  }
                  ...
                  if ( IMAT.EQ.6 .OR. IMAT.EQ.7 .OR. IMAT.EQ.8 .OR. IMAT.EQ.10 .OR. IMAT.EQ.11 ) {
                     ...
                     DO J = 1, JB_ZERO-1, 1
                        CALL ZSWAP( M, COPYA( ( NB_ZERO+J-1)*LDA+1), 1, COPYA( (J-1)*LDA + 1 ), 1 )
                     END DO
                  ...
                  // 5) Order the singular values generated by
                    ...
                    t // hat correspond to zero columns.
               ...
               DO INB = 1, NNB
                  ...
                  CALL ZLACPY( 'All', M, NRHS, COPYB, LDA, A( LDA*N + 1 ),  LDA )                   CALL ZLACPY( 'All', M, NRHS, COPYB, LDA, B,  LDA )
                  ...
                  SRNAMT = 'ZGEQP3RK'
                  CALL ZGEQP3RK( M, N, NRHS, KMAX, ABSTOL, RELTOL, A, LDA, KFACT, MAXC2NRMK, RELMAXC2NRMK, IWORK( N+1 ), TAU, WORK, LW, RWORK, IWORK( 2*N+1 ), INFO )
                 ...
                 t // he threshold.
                  ...
                  if ( MINMN.GT.0 ) {
                     ...
                     CALL ZUNMQR( 'Left', 'Conjugate transpose', M, NRHS, KFACT, A, LDA, TAU, B, LDA, WORK, LWORK_MQR, INFO )

                     DO I = 1, NRHS

                        // Compare N+J-th column of A and J-column of B.

                        CALL ZAXPY( M, -CONE, A( ( N+I-1 )*LDA+1 ), 1, B( ( I-1 )*LDA+1 ), 1 )
                     END DO
