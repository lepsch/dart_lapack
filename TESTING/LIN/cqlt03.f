...
// =====================================================================
      ...
      if (K > 0 && M > K) CALL CLACPY( 'Full', M-K, K, AF( 1, N-K+1 ), LDA, Q( 1, M-K+1 ), LDA )       IF( K > 1 ) CALL CLACPY( 'Upper', K-1, K-1, AF( M-K+1, N-K+2 ), LDA, Q( M-K+1, M-K+2 ), LDA );
      ...
      for (ISIDE = 1; ISIDE <= 2; ISIDE++) { // 30
         ...
         for (ITRANS = 1; ITRANS <= 2; ITRANS++) { // 20
            ...
            SRNAMT = 'CUNMQL';
            if (K > 0) CALL CUNMQL( SIDE, TRANS, MC, NC, K, AF( 1, N-K+1 ), LDA, TAU( MINMN-K+1 ), CC, LDA, WORK, LWORK, INFO );
