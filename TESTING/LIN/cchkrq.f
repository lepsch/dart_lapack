...
*                       Test CGERQF
*
                        CALL CRQT01( M, N, A, AF, AQ, AR, LDA, TAU, WORK, LWORK, RWORK, RESULT( 1 ) )
                     ELSE IF( M.LE.N ) THEN
*
*                       Test CUNGRQ, using factorization
*                       returned by CRQT01
*
                        CALL CRQT02( M, N, K, A, AF, AQ, AR, LDA, TAU, WORK, LWORK, RWORK, RESULT( 1 ) )
                     END IF
                     IF( M.GE.K ) THEN
*
...
*                       by CRQT01
*
                        CALL CRQT03( M, N, K, AF, AC, AR, AQ, LDA, TAU, WORK, LWORK, RWORK, RESULT( 3 ) )
                        NT = NT + 4
*
...
*
                           CALL CLACPY( 'Full', M, NRHS, B, LDA, X( N-M+1 ), LDA )
                           SRNAMT = 'CGERQS'
                           CALL CGERQS( M, N, NRHS, AF, LDA, TAU, X, LDA, WORK, LWORK, INFO )
*
*                          Check error code from CGERQS.
*
                           IF( INFO.NE.0 ) CALL ALAERH( PATH, 'CGERQS', INFO, 0, ' ', M, N, NRHS, -1, NB, IMAT, NFAIL, NERRS, NOUT )
*
