...
*  =====================================================================
      ...
      for (IN = 1; IN <= NN; IN++) { // 140
         ...
         for (IMAT = 1; IMAT <= NIMAT; IMAT++) { // 130
            ...
            if (ZEROT && N.LT.IMAT-2) GO TO 130;
            ...
            for (IUPLO = 1; IUPLO <= 2; IUPLO++) { // 120
               ...
               for (IEQUED = 1; IEQUED <= 2; IEQUED++) { // 110
                  ...
                  for (IFACT = 1; IFACT <= NFACT; IFACT++) { // 100
                     ...
                     if ( NOFACT ) {
                        ...
                        for (K = 1; K <= NT; K++) { // 60
                           if ( RESULT( K ).GE.THRESH ) {
                              if (NFAIL == 0 && NERRS == 0) CALL ALADHD( NOUT, PATH )                               WRITE( NOUT, FMT = 9999 )'DPPSV ', UPLO, N, IMAT, K, RESULT( K );
                              NFAIL = NFAIL + 1
                           }
                        } // 60
                        NRUN = NRUN + NT
                     ...
                     if (.NOT.PREFAC && NPP.GT.0) CALL DLASET( 'Full', NPP, 1, ZERO, ZERO, AFAC, NPP );
