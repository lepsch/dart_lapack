      ...
      for (IN = 1; IN <= NN; IN++) { // 120
         ...
         for (IMAT = 1; IMAT <= NIMAT; IMAT++) { // 110
            ...
            for (IFACT = 1; IFACT <= 2; IFACT++) { // 100
               ...
               if ( IFACT == 2 ) {
                  ...
                  for (K = 1; K <= NT; K++) { // 70
                     if ( RESULT( K ).GE.THRESH ) {
                        if (NFAIL == 0 .AND. NERRS == 0) CALL ALADHD( NOUT, PATH )                         WRITE( NOUT, FMT = 9999 )'CPTSV ', N, IMAT, K, RESULT( K );
                        NFAIL = NFAIL + 1
                     }
                  } // 70
                  NRUN = NRUN + NT
               ...
               for (K = K1; K <= 6; K++) { // 90
                  if ( RESULT( K ).GE.THRESH ) {
                     if (NFAIL == 0 .AND. NERRS == 0) CALL ALADHD( NOUT, PATH )                      WRITE( NOUT, FMT = 9998 )'CPTSVX', FACT, N, IMAT, K, RESULT( K );
                     NFAIL = NFAIL + 1
                  }
               } // 90
               NRUN = NRUN + 7 - K1
