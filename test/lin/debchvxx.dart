      void debchvxx(final int THRESH, final int PATH,) {
      // .. Scalar Arguments ..
      double            THRESH;
      String            PATH;
      ...
      // .. Local Arrays ..
      double             TSTRAT(NTESTS), RINV(NMAX), PARAMS(NPARAMS), S(NMAX),R(NMAX),C(NMAX), DIFF(NMAX, NMAX), ERRBND_N(NMAX*3), ERRBND_C(NMAX*3), A(NMAX,NMAX),INVHILB(NMAX,NMAX),X(NMAX,NMAX), AB( (NMAX-1)+(NMAX-1)+1, NMAX ), ABCOPY( (NMAX-1)+(NMAX-1)+1, NMAX ), AFB( 2*(NMAX-1)+(NMAX-1)+1, NMAX ), WORK(NMAX*3*5), AF(NMAX, NMAX),B(NMAX, NMAX), ACOPY(NMAX, NMAX);
...
// Create the loop to test out the Hilbert matrices
      ...
      for (N = 1; N <= NMAX; N++) {
         ...
         if ( lsamen( 2, C2, 'SY' ) ) {
            dsysvxx(FACT, UPLO, N, NRHS, ACOPY, LDA, AF, LDA, IPIV, EQUED, S, B, LDA, X, LDA, ORCOND, RPVGRW, BERR, NERRBND, ERRBND_N, ERRBND_C, NPARAMS, PARAMS, WORK, IWORK, INFO);
         } else if ( lsamen( 2, C2, 'PO' ) ) {
            dposvxx(FACT, UPLO, N, NRHS, ACOPY, LDA, AF, LDA, EQUED, S, B, LDA, X, LDA, ORCOND, RPVGRW, BERR, NERRBND, ERRBND_N, ERRBND_C, NPARAMS, PARAMS, WORK, IWORK, INFO);
         } else if ( lsamen( 2, C2, 'GB' ) ) {
            dgbsvxx(FACT, TRANS, N, KL, KU, NRHS, ABCOPY, LDAB, AFB, LDAFB, IPIV, EQUED, R, C, B, LDA, X, LDA, ORCOND, RPVGRW, BERR, NERRBND, ERRBND_N, ERRBND_C, NPARAMS, PARAMS, WORK, IWORK, INFO);
         } else {
            dgesvxx(FACT, TRANS, N, NRHS, ACOPY, LDA, AF, LDA, IPIV, EQUED, R, C, B, LDA, X, LDA, ORCOND, RPVGRW, BERR, NERRBND, ERRBND_N, ERRBND_C, NPARAMS, PARAMS, WORK, IWORK, INFO);
         }
...
// $$$         WRITE(*,*)
 ...
 9999 FORMAT( ' D${.a2}SVXX: N =${.i2}, RHS = ${.i2}, NWISE GUAR. = ${}, CWISE GUAR. = ${} test(',I1,') =${.g12_5};
 ...
 8000 FORMAT( ' D${.a2}SVXX: N =${.i2}, INFO = ${.i3}, ORCOND = ${.g12_5}, real RCOND = ${.g12_5};
      }
