...
void debchvxx(final double THRESH, final String PATH, final Nout NOUT) {
  ...
  final BERR = Array<double>(NMAX),
      ...
      INVHILB = Matrix<double>(NMAX, NMAX),
      X = Matrix<double>(NMAX, NMAX),
      AB = Matrix<double>((NMAX - 1) + (NMAX - 1) + 1, NMAX),
      ABCOPY = Matrix<double>((NMAX - 1) + (NMAX - 1) + 1, NMAX),
      AFB = Matrix<double>(2 * (NMAX - 1) + (NMAX - 1) + 1, NMAX),
      WORK = Array<double>(NMAX * 3 * 5),
      AF = Matrix<double>(NMAX, NMAX),
      B = Matrix<double>(NMAX, NMAX),
      ACOPY = Matrix<double>(NMAX, NMAX);
  final IPIV = Array<int>(NMAX), IWORK = Array<int>(3 * NMAX);
