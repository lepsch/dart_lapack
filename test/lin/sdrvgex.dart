      void sdrvge(DOTYPE, NN, NVAL, NRHS, THRESH, TSTERR, NMAX, A, AFAC, ASAV, B, BSAV, X, XACT, S, final Array<double> _WORK, final Array<double> RWORK, final Array<int> IWORK, final int NOUT) {
               ...
               for (IFACT = 1; IFACT <= NFACT; IFACT++) { // 60
                  ...
                  for (ITRAN = 1; ITRAN <= NTRAN; ITRAN++) { // 50
                    ...
                    srnamc.SRNAMT = 'SGESVXX';
                     N_ERR_BNDS = 3;
                     sgesvxx(FACT, TRANS, N, NRHS, A, LDA, AFAC, LDA, IWORK, EQUED, S, S( N+1 ), B, LDA, X, LDA, RCOND, RPVGRW_SVXX, BERR, N_ERR_BNDS, ERRBNDS_N, ERRBNDS_C, 0, ZERO, WORK, IWORK( N+1 ), INFO );

                     // Check the error code from SGESVXX.

                     if (INFO == N+1) GOTO 50;
                     if ( INFO != IZERO ) {
                        alaerh(PATH, 'SGESVXX', INFO, IZERO, FACT + TRANS, N, N, -1, -1, NRHS, IMAT, NFAIL, NERRS, NOUT );
                        GOTO 50;
                     }
                     ...
                     if ( INFO > 0 && INFO < N+1 ) {
                        RPVGRW = SLA_GERPVGRW (N, INFO, A, LDA, AFAC, LDA);
                     } else {
                     ...
                     if ( !TRFCON ) {
                        for (K = K1; K <= NTESTS; K++) { // 45
                           if ( RESULT( K ) >= THRESH ) {
                              ...
                              if ( PREFAC ) {
                                 ...
                                 WRITE( NOUT, FMT = 9998 )'SGESVXX', FACT, TRANS, N, IMAT, K, RESULT( K );
                              }
                              NFAIL = NFAIL + 1;
                           }
                        } // 45
                        ...
                        if( RESULT( 1 ) >= THRESH && !PREFAC ) {
                           ...
                           if ( PREFAC ) {
                              ...
                              WRITE( NOUT, FMT = 9998 )'SGESVXX', FACT, TRANS, N, IMAT, 1, RESULT( 1 );
                           }
                           NFAIL = NFAIL + 1;
                           NRUN = NRUN + 1;
                        }
                        if ( RESULT( 6 ) >= THRESH ) {
                           ...
                           if ( PREFAC ) {
                              ...
                              WRITE( NOUT, FMT = 9998 )'SGESVXX', FACT, TRANS, N, IMAT, 6, RESULT( 6 );
                           }
                           NFAIL = NFAIL + 1;
                           NRUN = NRUN + 1;
                        }
                        if ( RESULT( 7 ) >= THRESH ) {
                           ...
                           if ( PREFAC ) {
                              ...
                              WRITE( NOUT, FMT = 9998 )'SGESVXX', FACT, TRANS, N, IMAT, 7, RESULT( 7 );
                           }
                           NFAIL = NFAIL + 1;
                           NRUN = NRUN + 1;
                        }
