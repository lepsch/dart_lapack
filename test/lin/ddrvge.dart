...
// -- Univ. of California Berkeley, Univ. of Colorado Denver and NAG Ltd..--
  ...
  const NTRAN = 3;
  int NFAIL, NRUN;
  double RCONDI = 0, RCONDO = 0, ROLDI = 0, ROLDO = 0;
      ...
      for (var IEQUED = 1; IEQUED <= 4; IEQUED++) {
        ...
        for (var IFACT = 1; IFACT <= NFACT; IFACT++) {
          ...
          for (var ITRAN = 1; ITRAN <= NTRAN; ITRAN++) {
            ...
            if (!PREFAC) dlaset('Full', N, N, ZERO, ZERO, AFAC.asMatrix(), LDA);
            ...
            if (INFO.value != IZERO) {
              alaerh(PATH, 'DGESVX', INFO.value, IZERO, FACT + TRANS, N, N, -1,
            ...
            if (INFO.value != 0 && INFO.value <= N) {
              ...
              RPVGRW = dlantr('M', 'U', 'N', N, N, AFAC.asMatrix(), LDA, WORK);
              if (RPVGRW == ZERO) {
                RPVGRW = ONE;
              } else {
            ...
            if (INFO.value == 0) {
              ...
              dget02(TRANS, N, N, NRHS, ASAV.asMatrix(), LDA, X.asMatrix(), LDA,
            ...
            if (!TRFCON) {
              ...
              if (RESULT[1] >= THRESH && !PREFAC) {
                if (NFAIL == 0 && NERRS.value == 0) aladhd(NOUT, PATH);
                if (PREFAC) {
                  NOUT.print9997('DGESVX', FACT, TRANS, N, EQUED.value, IMAT, 1,
                      RESULT[1]);
                } else {
              ...
              if (RESULT[6] >= THRESH) {
                if (NFAIL == 0 && NERRS.value == 0) aladhd(NOUT, PATH);
                if (PREFAC) {
                  NOUT.print9997('DGESVX', FACT, TRANS, N, EQUED.value, IMAT, 6,
                      RESULT[6]);
                } else {
              ...
              if (RESULT[7] >= THRESH) {
                if (NFAIL == 0 && NERRS.value == 0) aladhd(NOUT, PATH);
                if (PREFAC) {
                  NOUT.print9997('DGESVX', FACT, TRANS, N, EQUED.value, IMAT, 7,
                      RESULT[7]);
                } else {
