...
// -- Univ. of California Berkeley, Univ. of Colorado Denver and NAG Ltd..--
  ...
  test.group('error exits', () {
    ...
    if (TSTERR) derrls(PATH, NOUT, test);
    test.tearDown(() {
      infoc.INFOT = 0;
    });
  });
          ...
          for (var ISCALE = 1; ISCALE <= 3; ISCALE++) {
            ...
            if (IRANK == 1) {
              for (var ITRAN = 1; ITRAN <= 2; ITRAN++) {
                ...
                dgelst(TRANS, M, N, NRHS, A.asMatrix(), LDA, B.asMatrix(), LDB,
                    WQ, -1, INFO);
                LWORK_DGELST = WQ[1].toInt();
                // Compute workspace needed for DGETSLS
                dgetsls(TRANS, M, N, NRHS, A.asMatrix(), LDA, B.asMatrix(), LDB,
                    WQ, -1, INFO);
  ...
  for (final IM in 1.through(NM)) {
    ...
    for (final IN in 1.through(NN)) {
      ...
      for (final INS in 1.through(NNS)) {
        ...
        for (final IRANK in 1.through(2)) {
          for (final ISCALE in 1.through(3)) {
            final ITYPE = (IRANK - 1) * 3 + ISCALE;

            final skip = !DOTYPE[ITYPE];
            test('DDRVLS (IM=$IM IN=$IN INS=$INS IRANK=$IRANK ISCALE=$ISCALE)',
                () {
              final INFO = Box(0);
              ...
              if (IRANK == 1) {
                ...
                for (var INB = 1; INB <= NNB; INB++) {
                  ...
                  for (var ITRAN = 1; ITRAN <= 2; ITRAN++) {
                    ...
                    srnamc.SRNAMT = 'DGELS';
                    dgels(TRANS, M, N, NRHS, A.asMatrix(), LDA, B.asMatrix(),
                        LDB, WORK, LWORK, INFO);

                    test.expect(INFO.value, 0, reason: 'DGELS');
                    if (INFO.value != 0) {
                      alaerh(PATH, 'DGELS ', INFO.value, 0, TRANS, M, N, NRHS,
                    ...
                    dqrt16(TRANS, M, N, NRHS, COPYA.asMatrix(), LDA,
                        B.asMatrix(), LDB, C.asMatrix(), LDB, WORK, RESULT(1));

                    // Test 2: Check correctness of results for DGELS.
                    if ((ITRAN == 1 && M >= N) || (ITRAN == 2 && M < N)) {
                      ...
                      RESULT[2] = dqrt14(TRANS, M, N, NRHS, COPYA.asMatrix(),
                ...
                for (var INB = 1; INB <= NNB; INB++) {
                  ...
                  for (var ITRAN = 1; ITRAN <= 2; ITRAN++) {
                    ...
                    dgelst(TRANS, M, N, NRHS, A.asMatrix(), LDA, B.asMatrix(),
                        LDB, WORK, LWORK, INFO);
                    test.expect(INFO.value, 0, reason: 'DGELST');
                    if (INFO.value != 0) {
                      alaerh(PATH, 'DGELST', INFO.value, 0, TRANS, M, N, NRHS,
                          -1, NB, ITYPE, NFAIL, NERRS, NOUT);
                    }
                    ...
                    dqrt16(TRANS, M, N, NRHS, COPYA.asMatrix(), LDA,
                        B.asMatrix(), LDB, C.asMatrix(), LDB, WORK, RESULT(3));

                    // Test 4: Check correctness of results for DGELST.
                    if ((ITRAN == 1 && M >= N) || (ITRAN == 2 && M < N)) {
                      // Solving LS system, compute:
                      // r = norm((B- A*X)**T * A) /
                      //  / (norm(A)*norm(B)*max(M,N,NRHS)*EPS)
                      ...
                      // Solving overdetermined system
                      RESULT[4] = dqrt14(TRANS, M, N, NRHS, COPYA.asMatrix(),
                          LDA, B.asMatrix(), LDB, WORK, LWORK);
                    }
                ...
                for (var IMB = 1; IMB <= NNB; IMB++) {
                  ...
                  for (var INB = 1; INB <= NNB; INB++) {
                    ...
                    for (var ITRAN = 1; ITRAN <= 2; ITRAN++) {
                      ...
                      // Solve LS or overdetermined system
                      if (M > 0 && N > 0) {
                        dlacpy('Full', M, N, COPYA.asMatrix(), LDA,
                            A.asMatrix(), LDA);
                        dlacpy('Full', NROWS, NRHS, COPYB.asMatrix(), LDB,
                            B.asMatrix(), LDB);
                      }
                      srnamc.SRNAMT = 'DGETSLS';
                      dgetsls(TRANS, M, N, NRHS, A.asMatrix(), LDA,
                          B.asMatrix(), LDB, WORK, LWORK, INFO);
                      test.expect(INFO.value, 0, reason: 'DGETSLS');
                      if (INFO.value != 0) {
                        alaerh(PATH, 'DGETSLS', INFO.value, 0, TRANS, M, N,
                            NRHS, -1, NB, ITYPE, NFAIL, NERRS, NOUT);
                      }
                      ...
                      // Test 6: Check correctness of results for DGETSLS.
                      if ((ITRAN == 1 && M >= N) || (ITRAN == 2 && M < N)) {
                        // Solving LS system, compute:
                        // r = norm((B- A*X)**T * A) /
                        // / (norm(A)*norm(B)*max(M,N,NRHS)*EPS)
              ...
              for (var INB = 1; INB <= NNB; INB++) {
                ...
                dgelss(M, N, NRHS, A.asMatrix(), LDA, B.asMatrix(), LDB, S,
                    RCOND, CRANK, WORK, LWORK, INFO);
                test.expect(INFO.value, 0, reason: 'DGELSS');
                if (INFO.value != 0) {
                ...
                test.expect(INFO.value, 0, reason: 'DGELSD');
                if (INFO.value != 0) {
