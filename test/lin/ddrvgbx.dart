import 'common.dart';
      ...
      void ddrvgb(DOTYPE, NN, NVAL, NRHS, THRESH, TSTERR, A, LA, AFB, LAFB, ASAV, B, BSAV, X, XACT, S, final Array<double> _WORK, final Array<double> RWORK, final Array<int> IWORK, final int NOUT) {
               ...
               for (IMAT = 1; IMAT <= NIMAT; IMAT++) { // 120
                  ...
                  for (IEQUED = 1; IEQUED <= 4; IEQUED++) { // 110
                     ...
                     for (IFACT = 1; IFACT <= NFACT; IFACT++) { // 100
                        ...
                        for (ITRAN = 1; ITRAN <= NTRAN; ITRAN++) { // 90
                           ...
                           if ( NOFACT && ITRAN == 1 ) {
                              ...
                              if (INFO != IZERO) alaerh( PATH, 'DGBSV ', INFO, IZERO, ' ', N, N, KL, KU, NRHS, IMAT, NFAIL, NERRS, NOUT );
                              ...
                              for (K = 1; K <= NT; K++) { // 50
                                 if ( RESULT( K ) >= THRESH ) {
                                    if (NFAIL == 0 && NERRS == 0) aladhd( NOUT, PATH );
                                    WRITE( NOUT, FMT = 9997 )'DGBSV ', N, KL, KU, IMAT, K, RESULT( K );
                                    NFAIL = NFAIL + 1;
                                 }
                              } // 50
                              NRUN = NRUN + NT;
                           ...
                           if ( !PREFAC) dlaset( 'Full', 2*KL+KU+1, N, ZERO, ZERO, AFB, LDAFB );
                     ...
                     if ( INFO == 0 ) {
                        ...
                        dlacpy('Full', N, NRHS, BSAV, LDB, WORK, LDB );
                        dgbt02(TRANS, N, N, KL, KU, NRHS, ASAV, LDA, X, LDB, WORK, LDB, RWORK, RESULT( 2 ) );
