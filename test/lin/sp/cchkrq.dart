      void cchkrq(final int DOTYPE, final int NM, final int MVAL, final int NN, final int NVAL, final int NNB, final int NBVAL, final int NXVAL, final int NRHS, final int THRESH, final int TSTERR, final int NMAX, final int A, final int AF, final int AQ, final int AR, final int AC, final int B, final int X, final int XACT, final int TAU, final Array<double> _WORK_, final Array<double> RWORK_, final Array<int> IWORK_, final int NOUT,) {
...
// -- Univ. of California Berkeley, Univ. of Colorado Denver and NAG Ltd..--
      ...
      // EXTERNAL ALAERH, ALAHD, ALASUM, CERRRQ, CGERQS, CGET02, CLACPY, CLARHS, CLATB4, CLATMS, CRQT01, CRQT02, CRQT03, XLAENV
      // ..
      // .. Intrinsic Functions ..
      // INTRINSIC MAX, MIN
      // ..
               ...
               for (IK = 1; IK <= NK; IK++) { // 40
                  ...
                  for (INB = 1; INB <= NNB; INB++) { // 30
                     ...
                     if ( IK == 1 ) {
                        ...
                        crqt01(M, N, A, AF, AQ, AR, LDA, TAU, WORK, LWORK, RWORK, RESULT( 1 ) );
                     } else if ( M <= N ) {

                        // Test CUNGRQ, using factorization
                        // returned by CRQT01

                        crqt02(M, N, K, A, AF, AQ, AR, LDA, TAU, WORK, LWORK, RWORK, RESULT( 1 ) );
                     }
                     if ( M >= K ) {
                        ...
                        crqt03(M, N, K, AF, AC, AR, AQ, LDA, TAU, WORK, LWORK, RWORK, RESULT( 3 ) );
                        NT = NT + 4;
                        ...
                        if ( K == M && INB == 1 ) {
                          ...
                          srnamc.SRNAMT = 'CLARHS';
                           ...
                           clacpy('Full', M, NRHS, B, LDA, X( N-M+1 ), LDA );
                          srnamc.SRNAMT = 'CGERQS';
                           cgerqs(M, N, NRHS, AF, LDA, TAU, X, LDA, WORK, LWORK, INFO );

                           // Check error code from CGERQS.

                           if (INFO != 0) alaerh( PATH, 'CGERQS', INFO, 0, ' ', M, N, NRHS, -1, NB, IMAT, NFAIL, NERRS, NOUT );
