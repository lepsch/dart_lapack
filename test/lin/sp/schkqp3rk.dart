      void schkqp3rk(final int DOTYPE, final int NM, final int MVAL, final int NN, final int NVAL, final int NNS, final int NSVAL, final int NNB, final int NBVAL, final int NXVAL, final int THRESH, final int A, final int COPYA, final int B, final int COPYB, final int S, final int TAU, final Array<double> _WORK, final Array<int> IWORK, final int NOUT,) {
// -- LAPACK test routine --
         ...
         for (IN = 1; IN <= NN; IN++) {
            ...
            // 19. Random, CNDNUM = 2, scaled near overflow                            CNDNUM = 2                      LARGE = 1.0/( 0.25 * ( SAFMIN / EPS ) )  3 ( geometric distribution of singular values )
               ...
               for (INB = 1; INB <= NNB; INB++) {
                  ...
                  slacpy('All', M, NRHS, COPYB, LDA, A( LDA*N + 1 ),  LDA );
                 ...
                 srnamc.SRNAMT = 'SGEQP3RK';
                  ...
                  if (INFO < 0) alaerh( PATH, 'SGEQP3RK', INFO, 0, ' ', M, N, NX, -1, NB, IMAT, NFAIL, NERRS, NOUT );
                  ...
                  if ( KFACT == MINMN ) {
                     ...
                     for (T = 1; T <= 1; T++) {
                        if ( RESULT( T ) >= THRESH ) {
                           ...
                           WRITE( NOUT, FMT = 9999 ) 'SGEQP3RK', M, N, NRHS, KMAX, ABSTOL, RELTOL, NB, NX, IMAT, T, RESULT( T );
                           NFAIL = NFAIL + 1;
                        }
                     }
                     NRUN = NRUN + 1;
