      void clavhp(final int UPLO, final int TRANS, final int DIAG, final int N, final int NRHS, final int A, final Array<int> IPIV_, final Matrix<double> B_, final int LDB, final Box<int> INFO,) {
      ...
      if ( lsame( TRANS, 'N' ) ) {
         ...
         // Loop forward applying the transformations.
            ...
            if ( IPIV( K ) > 0 ) {
               ...
               if (NOUNIT) cscal( NRHS, A( KC+K-1 ), B( K, 1 ), LDB );
               ...
               if ( K > 1 ) {
                  ...
                  cgeru(K-1, NRHS, ONE, A( KC ), 1, B( K, 1 ), LDB, B( 1, 1 ), LDB );
                  cgeru(K-1, NRHS, ONE, A( KCNEXT ), 1, B( K+1, 1 ), LDB, B( 1, 1 ), LDB );
         ...
         // where L = P(1)*inv(L(1))* ... *P(m)*inv(L(m)) .
            ...
            if ( IPIV( K ) > 0 ) {
               ...
               if (NOUNIT) cscal( NRHS, A( KC ), B( K, 1 ), LDB );
               ...
               if ( K != N ) {
                  ...
                  cgeru(N-K, NRHS, ONE, A( KC+1 ), 1, B( K, 1 ), LDB, B( K+1, 1 ), LDB );
                  cgeru(N-K, NRHS, ONE, A( KCNEXT+2 ), 1, B( K-1, 1 ), LDB, B( K+1, 1 ), LDB );
