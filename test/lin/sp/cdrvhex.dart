      void cdrvhe(final int DOTYPE, final int NN, final int NVAL, final int NRHS, final int THRESH, final int TSTERR, final int NMAX, final int A, final int AFAC, final int AINV, final int B, final int X, final int XACT, final Array<double> _WORK_, final Array<double> RWORK_, final Array<int> IWORK_, final int NOUT,) {
...
// -- Univ. of California Berkeley, Univ. of Colorado Denver and NAG Ltd..--
      ...
      // EXTERNAL ALADHD, ALAERH, ALASVM, CERRVX, CGET04, CHESV, CHESVX, CHET01, CHETRF, CHETRI2, CLACPY, CLAIPD, CLARHS, CLASET, CLATB4, CLATMS, CPOT02, CPOT05, XLAENV, CHESVXX
      // ..
      // .. Scalars in Common ..
      bool               LERR, OK;
              ...
              srnamc.SRNAMT = 'CLATMS';
               ...
               for (IFACT = 1; IFACT <= NFACT; IFACT++) { // 150
                    ...
                    srnamc.SRNAMT = 'CHESV ';
                     ...
                     for (K = 1; K <= NT; K++) { // 110
                        if ( RESULT( K ) >= THRESH ) {
                           ...
                           WRITE( NOUT, FMT = 9999 )'CHESV ', UPLO, N, IMAT, K, RESULT( K );
                           NFAIL = NFAIL + 1;
                        }
                     } // 110
                     NRUN = NRUN + NT;
                 ...
                 srnamc.SRNAMT = 'CHESVX';
                  ...
                  if ( INFO == 0 ) {
                     if ( IFACT >= 2 ) {
                        ...
                        chet01(UPLO, N, A, LDA, AFAC, LDA, IWORK, AINV, LDA, RWORK( 2*NRHS+1 ), RESULT( 1 ) );
                        K1 = 1;
                     } else {
                        K1 = 2;
                     }
                     ...
                     clacpy('Full', N, NRHS, B, LDA, WORK, LDA );
                     cpot02(UPLO, N, NRHS, A, LDA, X, LDA, WORK, LDA, RWORK( 2*NRHS+1 ), RESULT( 2 ) );
                  ...
                  for (K = K1; K <= 6; K++) { // 140
                     if ( RESULT( K ) >= THRESH ) {
                        if (NFAIL == 0 && NERRS == 0) aladhd( NOUT, PATH );
                        WRITE( NOUT, FMT = 9998 )'CHESVX', FACT, UPLO, N, IMAT, K, RESULT( K );
                        NFAIL = NFAIL + 1;
                     }
                  } // 140
                  NRUN = NRUN + 7 - K1;
                 ...
                 srnamc.SRNAMT = 'CHESVXX';
                  N_ERR_BNDS = 3;
                  EQUED = 'N';
                  chesvxx(FACT, UPLO, N, NRHS, A, LDA, AFAC, LDA, IWORK, EQUED, WORK( N+1 ), B, LDA, X, LDA, RCOND, RPVGRW_SVXX, BERR, N_ERR_BNDS, ERRBNDS_N, ERRBNDS_C, 0, ZERO, WORK, RWORK(2*NRHS+1), INFO );
                  ...
                  for (K = K1; K <= 6; K++) { // 85
                     if ( RESULT( K ) >= THRESH ) {
                        if (NFAIL == 0 && NERRS == 0) aladhd( NOUT, PATH );
                        WRITE( NOUT, FMT = 9998 )'CHESVXX', FACT, UPLO, N, IMAT, K, RESULT( K );
                        NFAIL = NFAIL + 1;
                     }
                  } // 85
                  NRUN = NRUN + 7 - K1;
