...
void zchkhe_rook(
  ...
  const ZERO = 0.0, ONE = 1.0, ONEHALF = 0.5;
    ...
    for (var IMAT = 1; IMAT <= NIMAT; IMAT++) {
      ...
      for (var IUPLO = 1; IUPLO <= 2; IUPLO++) {
        ...
        for (var INB = 1; INB <= NNB; INB++) {
          ...
          zhetrf_rook(UPLO, N, AFAC.asMatrix(), LDA, IWORK, AINV, LWORK, INFO);
          ...
          if (INFO.value != K) {
            alaerh(PATH, 'ZHETRF_ROOK', INFO.value, K, UPLO, N, N, -1, -1, NB,
          ...
          zhet01_rook(UPLO, N, A.asMatrix(), LDA, AFAC.asMatrix(), LDA, IWORK,
          ...
          if (INB == 1 && !TRFCON) {
            ...
            zhetri_rook(UPLO, N, AINV.asMatrix(), LDA, IWORK, WORK, INFO);

            // Check error code from ZHETRI_ROOK and handle error.

            if (INFO.value != 0) {
              alaerh(PATH, 'ZHETRI_ROOK', INFO.value, -1, UPLO, N, N, -1, -1,
          ...
          if (INB <= 1) {
            ...
            if (TRFCON) {
              ...
              for (var IRHS = 1; IRHS <= NNS; IRHS++) {
                ...
                zhetrs_rook(UPLO, N, NRHS, AFAC.asMatrix(), LDA, IWORK,
                    X.asMatrix(), LDA, INFO);

                // Check error code from ZHETRS_ROOK and handle error.

                if (INFO.value != 0) {
                  alaerh(PATH, 'ZHETRS_ROOK', INFO.value, 0, UPLO, N, N, -1, -1,
            ...
            if (INFO.value != 0) {
              alaerh(PATH, 'ZHECON_ROOK', INFO.value, 0, UPLO, N, N, -1, -1, -1,
                  IMAT, NFAIL, NERRS, NOUT);
            }
            ...
            if (RESULT[7] >= THRESH) {
              if (NFAIL == 0 && NERRS.value == 0) alahd(NOUT, PATH);
              NOUT.println(
                  ' UPLO = \'${UPLO.a1}\', N =${N.i5},${' ' * 10} type ${IMAT.i2}, test ${7.i2}, ratio =${RESULT[8].g12_5}');
              NFAIL++;
            }
            NRUN++;
          }
