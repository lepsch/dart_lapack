...
import 'package:lapack/src/dgeqp3rk.dart';
      ...
      for (var INS = 1; INS <= NNS; INS++) {
        ...
        for (var IMAT = 1; IMAT <= NTYPES; IMAT++) {
          ...
          if (IMAT == 1) {
            ...
            if (INFO.value != 0) {
              alaerh(PATH, 'DLATMS', INFO.value, 0, ' ', M, NB_GEN, -1, -1, -1,
          ...
          for (var INB = 1; INB <= NNB; INB++) {
            ...
            for (var KMAX = 0; KMAX <= min(M, N) + 1; KMAX++) {
              ...
              dlacpy('All', M, NRHS, COPYB.asMatrix(), LDA, B.asMatrix(), LDA);
              ...
              if (MINMN > 0) {
                ...
                dormqr('Left', 'Transpose', M, NRHS, KFACT.value, A.asMatrix(),
                    LDA, TAU, B.asMatrix(), LDA, WORK, LWORK_MQR, INFO);
