      void sdrvsy(final int DOTYPE, final int NN, final int NVAL, final int NRHS, final int THRESH, final int TSTERR, final int NMAX, final int A, final int AFAC, final int AINV, final int B, final int X, final int XACT, final Array<double> _WORK_, final Array<double> RWORK_, final Array<int> IWORK_, final int NOUT,) {
              ...
              srnamc.SRNAMT = 'SLATMS';
               ...
               for (IFACT = 1; IFACT <= NFACT; IFACT++) { // 150
                 ...
                 srnamc.SRNAMT = 'SLARHS';
                    ...
                    srnamc.SRNAMT = 'SSYSV ';
                     ...
                     for (K = 1; K <= NT; K++) { // 110
                        if ( RESULT( K ) >= THRESH ) {
                           ...
                           WRITE( NOUT, FMT = 9999 )'SSYSV ', UPLO, N, IMAT, K, RESULT( K );
                           NFAIL = NFAIL + 1;
                        }
                     } // 110
                     NRUN = NRUN + NT;
                  ...
                  if (IFACT == 2) slaset( UPLO, N, N, ZERO, ZERO, AFAC, LDA );
                 ...
                 srnamc.SRNAMT = 'SSYSVX';
                  ...
                  for (K = K1; K <= 6; K++) { // 140
                     if ( RESULT( K ) >= THRESH ) {
                        if (NFAIL == 0 && NERRS == 0) aladhd( NOUT, PATH );
                        WRITE( NOUT, FMT = 9998 )'SSYSVX', FACT, UPLO, N, IMAT, K, RESULT( K );
                        NFAIL = NFAIL + 1;
                     }
                  } // 140
                  NRUN = NRUN + 7 - K1;
                 ...
                 srnamc.SRNAMT = 'SSYSVXX';
                  ...
                  for (K = K1; K <= 6; K++) { // 85
                     if ( RESULT( K ) >= THRESH ) {
                        if (NFAIL == 0 && NERRS == 0) aladhd( NOUT, PATH );
                        WRITE( NOUT, FMT = 9998 )'SSYSVXX', FACT, UPLO, N, IMAT, K, RESULT( K );
                        NFAIL = NFAIL + 1;
                     }
                  } // 85
                  NRUN = NRUN + 7 - K1;
