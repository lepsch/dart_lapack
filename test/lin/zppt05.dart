      void zppt05(final int UPLO, final int N, final int NRHS, final int AP, final Matrix<double> B_, final int LDB, final Matrix<double> X_, final int LDX, final Matrix<double> XACT_, final int LDXACT, final int FERR, final int BERR, final int RESLTS,) {
  final B = B_.dim();
  final X = X_.dim();
      ...
      for (K = 1; K <= NRHS; K++) { // 90
         for (I = 1; I <= N; I++) { // 80
            ...
            if ( UPPER ) {
               ...
               TMP = TMP + ABS( (AP( JC+I )).toDouble() )*CABS1( X( I, K ) );
               JC = JC + I + I;
               for (J = I + 1; J <= N; J++) { // 50
               ...
               TMP = TMP + ABS( (AP( JC )).toDouble() )*CABS1( X( I, K ) );
               for (J = I + 1; J <= N; J++) { // 70
