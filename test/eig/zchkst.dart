  ...
  for (JSIZE = 1; JSIZE <= NSIZES; JSIZE++) {
    ...
    for (JTYPE = 1; JTYPE <= MTYPES; JTYPE++) {
      ...
      while (true) {
        ...
        if (IINFO.value != 0) {
          _print9999(NOUNIT, 'ZHETRD(L)', IINFO.value, N, JTYPE, IOLDSD);
          INFO.value = (IINFO.value).abs();
          if (IINFO.value < 0) {
            return;
          } else {
        ...
        if (IINFO.value != 0) {
          _print9999(NOUNIT, 'ZUNGTR(L)', IINFO.value, N, JTYPE, IOLDSD);
          INFO.value = (IINFO.value).abs();
          if (IINFO.value < 0) {
            return;
          } else {
        ...
        zhet21(2, 'Lower', N, 1, A, LDA, SD, SE, U, LDU, V, LDU, TAU, WORK,
            RWORK, RESULT(3));
        zhet21(3, 'Lower', N, 1, A, LDA, SD, SE, U, LDU, V, LDU, TAU, WORK,
            RWORK, RESULT(4));
      ...
      for (JR = 1; JR <= NTEST; JR++) {
        ...
        if (RESULT[JR] >= THRESH) {
          ...
          if (NERRS == 0) {
            ...
            NOUNIT.println(' Matrix types (see ZCHKST for details): ');
            NOUNIT.println(
