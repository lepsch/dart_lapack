...
void main() async {
  ...
  final IDUMMY = Array<int>(2), ISEED = Array<int>(4), RJOBDATA = Array<int>(8);
  double ANORM,
      COND,
      CONDL,
      CONDR,
                ...
                for (final WHTSVD in [1, 2, 3, 4]) {
                  ...
                  for (final LWMINOPT in [1, 2]) {
                    ...
                    if (lsame(JOBZ, 'V')) {
                      ...
                      // returned in W
                      dgemm('N', 'N', M, K.value, K.value, ONE, X, LDX, W, LDW,
                          ZERO, Z1, LDZ);
                      TMP = ZERO;
                      for (final i in 1.through(K.value)) {
                        daxpy(M, -ONE, Z(1, i).asArray(), 1, Z1(1, i).asArray(),
                            1);
                    ...
                    if (lsame(JOBREF, 'R')) {
                      ...
                      dgemm('N', 'N', M, K.value, M, ONE, A, LDA, X, LDX, ZERO,
                          Z1, LDZ);
                      TMP = ZERO;
                      for (final i in 1.through(K.value)) {
                    ...
                    if (lsame(RESIDS, 'R')) {
                      ...
                      dgemm('N', 'N', M, K.value, M, ONE, A, LDA, Z, LDZ, ZERO,
                          Y1, M);
                      ...
                      while (i <= K.value) {
                        if (IEIG[i] == ZERO) {
                          ...
                          AB[1][2] = IEIG[i];
                          AB[2][2] = REIG[i];
                          dgemm('N', 'N', M, 2, 2, -ONE, Z(1, i), M, AB, 2, ONE,
                              Y1(1, i), M);
                    ...
                    if (TEST_QRDMD && (K_TRAJ == 1)) {
                      ...
                      if (lsame(WANTQ, 'Q') && lsame(WANTR, 'R')) {
                        ...
                        dgemm('N', 'N', M, N + 1, min(M, N + 1), -ONE, F1, LDF,
                            Y, LDY, ONE, F2, LDF);
