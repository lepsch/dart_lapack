      void cchkst2stg(final int NSIZES, final int NN, final int NTYPES, final Array<bool> DOTYPE, final Array<int> ISEED, final int THRESH, final int NOUNIT, final Matrix<double> A, final int LDA, final int AP, final int SD, final int SE, final int D1, final int D2, final int D3, final int D4, final int D5, final int WA1, final int WA2, final int WA3, final int WR, final Matrix<double> U, final int LDU, final int V, final int VP, final int TAU, final int Z, final Array<double> WORK, final int LWORK, final Array<int> RWORK, final int LRWORK, final Array<int> IWORK, final int LIWORK, final int RESULT, final Box<int> INFO ) {
...
// -- Univ. of California Berkeley, Univ. of Colorado Denver and NAG Ltd..--
      ...
      for (JSIZE = 1; JSIZE <= NSIZES; JSIZE++) { // 310
         ...
         for (JTYPE = 1; JTYPE <= MTYPES; JTYPE++) { // 300
            ...
            LH = max(1, 4*N);
            LW = LWORK - LH;
            chetrd_2stage('N', "U", N, V, LDU, SD, SE, TAU,  WORK, LH, WORK( LH+1 ), LW, IINFO );
            ...
            chetrd_2stage('N', "L", N, V, LDU, SD, SE, TAU,  WORK, LH, WORK( LH+1 ), LW, IINFO );
            ...
            INDRWK = INDE + N;
            scopy(N, SD, 1, D1, 1 );
            if (N > 0) scopy( N-1, SE, 1, RWORK( INDE ), 1 );
            claset('Full', N, N, CZERO, CONE, Z, LDU );
            ...
            // Compute D1 and Z
                  ...
                  cstemr('V', 'V', N, D5, RWORK, VL, VU, IL, IU, M, D1, Z, LDU, M, IWORK( 1 ), TRYRAC, RWORK( N+1 ), LRWORK-N, IWORK( 2*N+1 ), LIWORK-2*N, IINFO );
                  if ( IINFO != 0 ) {
 ...
 9999 FORMAT( ' CCHKST2STG: ${} returned INFO=${.i6}.\n${' ' * 9}N=${.i6}, JTYPE=${.i6}, ISEED=(${.i5(4, ',')})' );
 ...
 9988 FORMAT( ' Matrix order=${.i5}, type=${.i2}, seed=${i4(4, ',')}', ' result ${.i3} is', 1P, E10.3 );
