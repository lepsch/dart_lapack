...
void _zchk1(
  ...
  final X = X_.dim(NMAX);
  final XX = XX_.dim(NMAX * NMAX);
  final XS = XS_.dim(NMAX * NMAX);
  final Y = Y_.dim(NMAX);
  final YY = YY_.dim(NMAX * NMAX);
  final YS = YS_.dim(NMAX * NMAX);
  ...
  for (IN = 1; IN <= NIDIM; IN++) {
    ...
    for (IM = 1; IM <= 2; IM++) {
      ...
      for (IKU = 1; IKU <= NK; IKU++) {
        ...
        TRANSL = Complex.zero;
        zmake(SNAME.substring(1, 3), ' ', ' ', M, N, A, NMAX, AA, LDA, KL, KU,
            RESET, TRANSL);
        ...
        for (IC = 1; IC <= 3; IC++) {
          ...
          for (IX = 1; IX <= NINC; IX++) {
            ...
            TRANSL = HALF;
            zmake('GE', ' ', ' ', 1, NL, X.asMatrix(), 1, XX, INCX.abs(), 0,
                NL - 1, RESET, TRANSL);
            if (NL > 1) {
              X[NL ~/ 2] = Complex.zero;
            ...
            for (IY = 1; IY <= NINC; IY++) {
              ...
              for (IA = 1; IA <= NALF; IA++) {
                ...
                for (IB = 1; IB <= NBET; IB++) {
                  ...
                  TRANSL = Complex.zero;
                  zmake('GE', ' ', ' ', 1, ML, Y.asMatrix(), 1, YY, INCY.abs(),
                      0, ML - 1, RESET, TRANSL);
  ...
  for (IN = 1; IN <= NIDIM; IN++) {
    ...
    for (IK = 1; IK <= NK; IK++) {
      ...
      for (IC = 1; IC <= 2; IC++) {
        ...
        TRANSL = Complex.zero;
        zmake(SNAME.substring(1, 3), UPLO, ' ', N, N, A, NMAX, AA, LDA, K, K,
            RESET, TRANSL);
        ...
        for (IX = 1; IX <= NINC; IX++) {
          ...
          TRANSL = HALF;
          zmake('GE', ' ', ' ', 1, N, X.asMatrix(), 1, XX, INCX.abs(), 0, N - 1,
              RESET, TRANSL);
          if (N > 1) {
            X[N ~/ 2] = Complex.zero;
          ...
          for (IY = 1; IY <= NINC; IY++) {
            ...
            for (IA = 1; IA <= NALF; IA++) {
              ...
              for (IB = 1; IB <= NBET; IB++) {
                ...
                TRANSL = Complex.zero;
                zmake('GE', ' ', ' ', 1, N, Y.asMatrix(), 1, YY, INCY.abs(), 0,
                    N - 1, RESET, TRANSL);
      ...
      for (ICU = 1; ICU <= 2; ICU++) {
        ...
        for (ICT = 1; ICT <= 3; ICT++) {
          ...
          for (ICD = 1; ICD <= 2; ICD++) {
            ...
            TRANSL = Complex.zero;
            zmake(SNAME.substring(1, 3), UPLO, DIAG, N, N, A, NMAX, AA, LDA, K,
                K, RESET, TRANSL);
            ...
            for (IX = 1; IX <= NINC; IX++) {
              ...
              TRANSL = HALF;
              zmake('GE', ' ', ' ', 1, N, X.asMatrix(), 1, XX, INCX.abs(), 0,
                  N - 1, RESET, TRANSL);
              if (N > 1) {
                X[N ~/ 2] = Complex.zero;
                XX[1 + INCX.abs() * (N ~/ 2 - 1)] = Complex.zero;
  ...
  for (IN = 1; IN <= NIDIM; IN++) {
    ...
    for (IM = 1; IM <= 2; IM++) {
      ...
      for (IX = 1; IX <= NINC; IX++) {
        ...
        TRANSL = HALF;
        zmake('GE', ' ', ' ', 1, M, X.asMatrix(), 1, XX, INCX.abs(), 0, M - 1,
            RESET, TRANSL);
        if (M > 1) {
          X[M ~/ 2] = Complex.zero;
        ...
        for (IY = 1; IY <= NINC; IY++) {
          ...
          TRANSL = Complex.zero;
          zmake('GE', ' ', ' ', 1, N, Y.asMatrix(), 1, YY, INCY.abs(), 0, N - 1,
              RESET, TRANSL);
          if (N > 1) {
            Y[N ~/ 2] = Complex.zero;
          ...
          for (IA = 1; IA <= NALF; IA++) {
            ...
            TRANSL = Complex.zero;
            zmake(SNAME.substring(1, 3), ' ', ' ', M, N, A, NMAX, AA, LDA,
    ...
    for (IC = 1; IC <= 2; IC++) {
      ...
      for (IX = 1; IX <= NINC; IX++) {
        ...
        for (IA = 1; IA <= NALF; IA++) {
          ...
          TRANSL = Complex.zero;
          zmake(SNAME.substring(1, 3), UPLO, ' ', N, N, A, NMAX, AA, LDA, N - 1,
              N - 1, RESET, TRANSL);
      ...
      for (IX = 1; IX <= NINC; IX++) {
        ...
        for (IY = 1; IY <= NINC; IY++) {
          ...
          for (IA = 1; IA <= NALF; IA++) {
            ...
            TRANSL = Complex.zero;
            zmake(SNAME.substring(1, 3), UPLO, ' ', N, N, A, NMAX, AA, LDA,
