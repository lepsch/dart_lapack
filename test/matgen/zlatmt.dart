...
// -- Univ. of California Berkeley, Univ. of Colorado Denver and NAG Ltd..--
  ...
  int I,
      ...
      NC,
      UUB;
  bool CSYM = false, GIVENS, ILEXTR, ILTEMP, TOPDWN;
  final IINFO = Box(0);
  final REALC = Box(0.0);
  final DUMMY = Box(Complex.zero),
      EXTRA = Box(Complex.zero),
      S = Box(Complex.zero),
      ZTEMP = Box(Complex.zero);
          ...
          for (JC = min(M + JKU, N) - 1; JC >= 1 - JKL; JC--) {
            ...
            for (JCH = JC + JKL;
              ...
              zlarot(
                  ...
                  ZTEMP);
              if (ILTEMP) {
                zlartg(A[JCH - ISKEW * ICOL + IOFFST][ICOL], ZTEMP.value, REALC,
                    S, DUMMY);
                DUMMY.value = zlarnd(5, ISEED);
                C = REALC.value.toComplex() * DUMMY.value;
                S.value = S.value * DUMMY.value;
          ...
          for (JR = min(N + JKL, M) - 1; JR >= 1 - JKU; JR--) {
            ...
            for (JCH = JR + JKU;
              ...
              zlarot(
                  ...
                  ZTEMP);
              if (ILTEMP) {
                zlartg(A[IROW - ISKEW * JCH + IOFFST][JCH], ZTEMP.value, REALC,
                    S, DUMMY);
                DUMMY.value = zlarnd(5, ISEED);
                C = REALC.value.toComplex() * DUMMY.value;
                S.value = S.value * DUMMY.value;
      ...
      if (TOPDWN) {
        ...
        for (K = 1; K <= UUB; K++) {
          ...
          for (JC = 1; JC <= N - 1; JC++) {
            ...
            IL = min(JC + 1, K + 2);
            EXTRA.value = Complex.zero;
            ZTEMP.value = A[JC - ISKEW * (JC + 1) + IOFFG][JC + 1];
            ANGLE = TWOPI * dlarnd(1, ISEED);
            C = cos(ANGLE).toComplex() * zlarnd(5, ISEED);
            S.value = sin(ANGLE).toComplex() * zlarnd(5, ISEED);
            if (CSYM) {
              ...
              ZTEMP.value = ZTEMP.value.conjugate();
              CT = C.conjugate();
              ST = S.value.conjugate();
            }
            zlarot(false, JC > K, true, IL, C, S.value,
                A(IROW - ISKEW * JC + IOFFG, JC).asArray(), ILDA, EXTRA, ZTEMP);
            ...
            for (JCH = JC - K; -K < 0 ? JCH >= 1 : JCH <= 1; JCH += -K) {
              ...
              C = (REALC.value.toComplex() * DUMMY.value).conjugate();
              S.value = (-S.value * DUMMY.value).conjugate();
              ZTEMP.value = A[JCH - ISKEW * (JCH + 1) + IOFFG][JCH + 1];
              if (CSYM) {
                CT = C;
                ST = S.value;
              } else {
        ...
        for (K = 1; K <= UUB; K++) {
          ...
          for (JC = N - 1; JC >= 1; JC--) {
            ...
            IL = min(N + 1 - JC, K + 2);
            EXTRA.value = Complex.zero;
            ZTEMP.value = A[1 + (1 - ISKEW) * JC + IOFFG][JC];
            ANGLE = TWOPI * dlarnd(1, ISEED);
            C = cos(ANGLE).toComplex() * zlarnd(5, ISEED);
            S.value = sin(ANGLE).toComplex() * zlarnd(5, ISEED);
            if (CSYM) {
            ...
            zlarot(false, true, N - JC > K, IL, C, S.value,
                A((1 - ISKEW) * JC + IOFFG, JC).asArray(), ILDA, ZTEMP, EXTRA);
            ICOL = max(1, JC - K + 1);
            zlarot(
                true,
                false,
            ...
            for (JCH = JC + K; K < 0 ? JCH >= N - 1 : JCH <= N - 1; JCH += K) {
              ...
              C = REALC.value.toComplex() * DUMMY.value;
              S.value = S.value * DUMMY.value;
              ZTEMP.value = A[1 + (1 - ISKEW) * JCH + IOFFG][JCH];
              if (CSYM) {
                CT = C;
                ST = S.value;
              } else {
