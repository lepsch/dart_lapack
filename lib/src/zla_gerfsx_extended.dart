...
import 'package:dart_lapack/src/xblas/blas_zgemv2_x.dart';
import 'package:dart_lapack/src/xblas/blas_zgemv_x.dart';
import 'package:dart_lapack/src/zgetrs.dart';
import 'package:dart_lapack/src/zla_geamv.dart';
import 'package:dart_lapack/src/zla_lin_berr.dart';
import 'package:dart_lapack/src/zla_wwaddw.dart';

void zla_gerfsx_extended(
  final int PREC_TYPE,
  ...
  final int N_NORMS,
  final Matrix<double> ERRS_N_,
  final Matrix<double> ERRS_C_,
  ...
  for (J = 1; J <= NRHS; J++) {
    ...
    for (CNT = 1; CNT <= ITHRESH; CNT++) {
      ...
      if (Y_PREC_STATE == BASE_RESIDUAL) {
        zgemv(TRANS, N, N, -Complex.one, A, LDA, Y(1, J).asArray(), 1,
            Complex.one, RES, 1);
      ...
      zcopy(N, RES, 1, DY, 1);
      zgetrs(TRANS, N, 1, AF, LDAF, IPIV, DY.asMatrix(N), N, INFO);
    ...
    zcopy(N, B(1, J).asArray(), 1, RES, 1);
    zgemv(TRANS, N, N, -Complex.one, A, LDA, Y(1, J).asArray(), 1, Complex.one,
        RES, 1);
    ...
    zla_geamv(TRANS_TYPE, N, N, 1.0, A, LDA, Y(1, J).asArray(), 1, 1.0, AYB, 1);
