      void zpbrfs(final int UPLO, final int N, final int KD, final int NRHS, final Matrix<double> AB, final int LDAB, final Matrix<double> AFB, final int LDAFB, final Matrix<double> B, final int LDB, final Matrix<double> X, final int LDX, final int FERR, final int BERR, final Array<double> _WORK, final Array<double> RWORK, final Box<int> INFO,) {
      ...
      for (J = 1; J <= NRHS; J++) { // 140
         ...
         if ( UPPER ) {
            for (K = 1; K <= N; K++) { // 50
               ...
               RWORK[K] = RWORK( K ) + ABS( (AB( KD+1, K )).toDouble() )* XK + S;
            } // 50
         } else {
            for (K = 1; K <= N; K++) { // 70
               S = ZERO;
               XK = CABS1( X( K, J ) );
               RWORK[K] = RWORK( K ) + ABS( (AB( 1, K )).toDouble() )*XK;
               L = 1 - K;
