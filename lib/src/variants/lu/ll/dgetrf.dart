      void dgetrf(final int M, final int N, final Matrix<double> A_, final int LDA, final Array<int> IPIV_, final Box<int> INFO,) {
  final A = A_.dim();
      ...
      if ( NB <= 1 || NB >= min( M, N ) ) {
         ...
         for (J = 1; NB < 0 ? J >= min( M, N ) : J <= min( M, N ); J += NB) { // 20
            ...
            for (K = 1; NB < 0 ? K >= J-NB : K <= J-NB; K += NB) { // 30
               ...
               dtrsm('Left', 'Lower', 'No transpose', 'Unit', NB, JB, ONE, A( K, K ), LDA, A( K, J ), LDA );
