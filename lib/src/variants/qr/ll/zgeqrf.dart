      void zgeqrf(final int M, final int N, final Matrix<double> A_, final int LDA, final int TAU, final Array<double> WORK_, final int LWORK, final Box<int> INFO,) {
  final A = A_.dim();
      ...
      if ( NB >= NBMIN && NB < K && NX < K ) {
         ...
         for (I = 1; NB < 0 ? I >= K - NX : I <= K - NX; I += NB) { // 10
            ...
            for (J = 1; NB < 0 ? J >= I - NB : J <= I - NB; J += NB) { // 20
               ...
               zlarfb('Left', 'Transpose', 'Forward', 'Columnwise', M-J+1, IB, NB, A( J, J ), LDA, WORK(J), LBWORK, A( J, I ), LDA, WORK(LBWORK*NB+NT*NT+1), IB);
...
20          CONTINUE
            ...
            zgeqr2(M-I+1, IB, A( I, I ), LDA, TAU( I ), WORK(LBWORK*NB+NT*NT+1), IINFO );
            ...
            if ( I+IB <= N ) {
               ...
               zlarft('Forward', 'Columnwise', M-I+1, IB, A( I, I ), LDA, TAU( I ), WORK(I), LBWORK );
      ...
      if ( I <= K ) {
         ...
         if ( I != 1 ) {
             ...
             for (J = 1; NB < 0 ? J >= I - NB : J <= I - NB; J += NB) { // 30
                 ...
                 zlarfb('Left', 'Transpose', 'Forward', 'Columnwise', M-J+1, K-I+1, NB, A( J, J ), LDA, WORK(J), LBWORK, A( J, I ), LDA, WORK(LBWORK*NB+NT*NT+1), K-I+1);
30           CONTINUE
              zgeqr2(M-I+1, K-I+1, A( I, I ), LDA, TAU( I ), WORK(LBWORK*NB+NT*NT+1),IINFO );
      ...
      if ( M < N && I != 1) {
          ...
          if ( NT <= NB ) {
               zlarft('Forward', 'Columnwise', M-I+1, K-I+1, A( I, I ), LDA, TAU( I ), WORK(I), LBWORK );
          } else {
               zlarft('Forward', 'Columnwise', M-I+1, K-I+1, A( I, I ), LDA, TAU( I ), WORK(LBWORK*NB+1), NT );
          }
          ...
          for (J = 1; NB < 0 ? J >= K-NX : J <= K-NX; J += NB) { // 40
               ...
               IB = min( K-J+1, NB );
                zlarfb('Left', 'Transpose', 'Forward', 'Columnwise', M-J+1, N-M, IB, A( J, J ), LDA, WORK(J), LBWORK, A( J, M+1 ), LDA, WORK(LBWORK*NB+NT*NT+1), N-M);

40       CONTINUE

         if ( NT <= NB ) {
             zlarfb('Left', 'Transpose', 'Forward', 'Columnwise', M-J+1, N-M, K-J+1, A( J, J ), LDA, WORK(J), LBWORK, A( J, M+1 ), LDA, WORK(LBWORK*NB+NT*NT+1), N-M);
         } else {
             zlarfb('Left', 'Transpose', 'Forward', 'Columnwise', M-J+1, N-M, K-J+1, A( J, J ), LDA, WORK(LBWORK*NB+1), NT, A( J, M+1 ), LDA, WORK(LBWORK*NB+NT*NT+1), N-M);
         }
