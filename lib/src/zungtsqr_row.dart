      void zungtsqr_row(final int M, final int N, final int MB, final int NB, final Matrix<double> A_, final int LDA, final Matrix<double> T_, final int LDT, final Array<double> WORK_, final int LWORK, final Box<int> INFO,) {
  ...
  final WORK = WORK_.dim();
      ...
      for (KB = KB_LAST; -NBLOCAL < 0 ? KB >= 1 : KB <= 1; KB += -NBLOCAL) {
         ...
         if ( MB1-KB-KNB+1 == 0 ) {
            ...
            zlarfb_gett('N', 0, N-KB+1, KNB, T( 1, KB ), LDT, A( KB, KB ), LDA, DUMMY( 1, 1 ), 1, WORK, KNB );
         } else {
            zlarfb_gett('N', MB1-KB-KNB+1, N-KB+1, KNB, T( 1, KB ), LDT, A( KB, KB ), LDA, A( KB+KNB, KB), LDA, WORK, KNB );
