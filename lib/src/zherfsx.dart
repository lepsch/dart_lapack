...
void zherfsx(final String UPLO, final String EQUED, final int N, final int NRHS,
final Matrix<Complex> A_, final int LDA, final Matrix<Complex> AF_, final int LDAF,
final Array<int> IPIV_, final Array<double> S_, final Matrix<Complex> B_, final int LDB,
final Matrix<Complex> X_, final int LDX, final Box<double> RCOND, final Array<double> BERR_,
final int N_ERR_BNDS, final Matrix<double> ERR_BNDS_NORM_, final Matrix<double> ERR_BNDS_COMP_,
final int NPARAMS, final Array<double> PARAMS_, final Array<Complex> WORK_,
...
final PARAMS=PARAMS_.dim();
      ...
      if ( REF_TYPE != 0 ) {
         ...
         PREC_TYPE = ilaprec( 'E' );
          zla_herfsx_extended(PREC_TYPE, UPLO,  N, NRHS, A, LDA, AF, LDAF, IPIV,
          RCEQU, S, B, LDB, X, LDX, BERR, N_NORMS, ERR_BNDS_NORM, ERR_BNDS_COMP,
          WORK, RWORK, WORK(N+1), TRANSFER (RWORK(1:2*N), ( (ZERO, ZERO) ), N),
          RCOND.value, ITHRESH, RTHRESH, UNSTABLE_THRESH, IGNORE_CWISE, INFO );
      }
      ...
      // cwise error is at least CWISE_WRONG.
         ...
         for (J = 1; J <= NRHS; J++) {
            if ( ERR_BNDS_COMP[ J][LA_LINRX_ERR_I ] < CWISE_WRONG ) {
               RCOND_TMP = zla_hercond_x( UPLO, N, A, LDA, AF, LDAF, IPIV, X( 1, J ).asArray(), INFO, WORK, RWORK );
            } else {
               RCOND_TMP = 0.0;
            }
