...
import 'package:lapack/src/xerbla.dart';
      ...
      void dgbtrf(final int M, final int N, final int KL, final int KU, final Matrix<double> AB_, final int LDAB, final Array<int> IPIV_, final Box<int> INFO,) {
  final AB = AB_.dim();
         ...
         for (J = 1; NB < 0 ? J >= min( M, N ) : J <= min( M, N ); J += NB) { // 180
            ...
            for (JJ = J; JJ <= J + JB - 1; JJ++) { // 80
               ...
               if ( AB( KV+JP, JJ ) != ZERO ) {
                  ...
                  JM = min( JU, J+JB-1 );
                  if (JM > JJ) dger( KM, JM-JJ, -ONE, AB( KV+2, JJ ), 1, AB( KV, JJ+1 ), LDAB-1, AB( KV+1, JJ+1 ), LDAB-1 );
               } else {
               ...
               NW = min( JJ-J+1, I3 );
               if (NW > 0) dcopy( NW, AB( KV+KL+1-JJ+J, JJ ), 1, WORK31( 1, JJ-J+1 ), 1 );
            } // 80
            if ( J+JB <= N ) {
            ...
            for (JJ = J + JB - 1; JJ >= J; JJ--) { // 170
               ...
               NW = min( I3, JJ-J+1 );
               if (NW > 0) dcopy( NW, WORK31( 1, JJ-J+1 ), 1, AB( KV+KL+1-JJ+J, JJ ), 1 );
            } // 170
         } // 180
      }
