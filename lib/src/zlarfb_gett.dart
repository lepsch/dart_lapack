...
// -- Univ. of California Berkeley, Univ. of Colorado Denver and NAG Ltd..--
  ...
  if (N > K) {
    ...
    for (J = 1; J <= N - K; J++) {
      zcopy(K, A(1, K + J).asArray(), 1, WORK(1, J).asArray(), 1);
    }
    ...
    if (LNOTIDENT) {
      ...
      ztrmm('L', 'L', 'C', 'U', K, N - K, Complex.one, A, LDA, WORK, LDWORK);
    }
    ...
    if (M > 0) {
      zgemm('C', 'N', K, N - K, M, Complex.one, B, LDB, B(1, K + 1), LDB,
          Complex.one, WORK, LDWORK);
    }

    // col2_(4) Compute W2: = T * W2,
    // T is upper-triangular.

    ztrmm('L', 'U', 'N', 'N', K, N - K, Complex.one, T, LDT, WORK, LDWORK);

    // col2_(5) Compute B2: = B2 - V2 * W2 = B2 - B1 * W2,
    // V2 stored in B1.

    if (M > 0) {
      zgemm('N', 'N', M, N - K, K, -Complex.one, B, LDB, WORK, LDWORK,
          Complex.one, B(1, K + 1), LDB);
    }
    ...
    if (LNOTIDENT) {
      ...
      ztrmm('L', 'L', 'N', 'U', K, N - K, Complex.one, A, LDA, WORK, LDWORK);
    }
  ...
  for (J = 1; J <= K; J++) {
    zcopy(J, A(1, J).asArray(), 1, WORK(1, J).asArray(), 1);
  }
  ...
  if (LNOTIDENT) {
    ...
    ztrmm('L', 'L', 'C', 'U', K, K, Complex.one, A, LDA, WORK, LDWORK);
  }
  ...
  ztrmm('L', 'U', 'N', 'N', K, K, Complex.one, T, LDT, WORK, LDWORK);

  // col1_(4) Compute B1: = - V2 * W1 = - B1 * W1,
  // V2 = B1, W1 is upper-triangular with zeroes below the diagonal.

  if (M > 0) {
    ztrmm('R', 'U', 'N', 'N', M, K, -Complex.one, WORK, LDWORK, B, LDB);
  }
  ...
  if (LNOTIDENT) {
    ...
    ztrmm('L', 'L', 'N', 'U', K, K, Complex.one, A, LDA, WORK, LDWORK);
