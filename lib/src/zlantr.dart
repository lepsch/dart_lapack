      double zlantr(final int NORM, final int UPLO, final int DIAG, final int M, final int N, final Matrix<double> A_, final int LDA, final Array<double> WORK_,) {
  final A = A_.dim();
      ...
      if ( min( M, N ) == 0 ) {
         ...
         if ( lsame( UPLO, 'U' ) ) {
            ...
            if ( lsame( DIAG, 'U' ) ) {
               ...
               SUM = min( M, N );
               for (J = 1; J <= N; J++) { // 310
                  zlassq(M-J, A( min( M, J+1 ), J ), 1, SCALE, SUM );
               } // 310
            } else {
               SCALE = ZERO;
               SUM = ONE;
