      void zla_herfsx_extended(PREC_TYPE, UPLO, N, NRHS, final Matrix<double> A, final int LDA, final Matrix<double> AF, final int LDAF, IPIV, COLEQU, C, final Matrix<double> B, final int LDB, final Matrix<double> Y, final int LDY, BERR_OUT, N_NORMS, ERR_BNDS_NORM, ERR_BNDS_COMP, RES, AYB, DY, Y_TAIL, RCOND, ITHRESH, RTHRESH, DZ_UB, IGNORE_CWISE, final Box<int> INFO ) {
...
// =====================================================================
      ...
      // EXTERNAL ZAXPY, ZCOPY, ZHETRS, ZHEMV, BLAS_ZHEMV_X, BLAS_ZHEMV2_X, ZLA_HEAMV, ZLA_WWADDW, ZLA_LIN_BERR
      double             DLAMCH;
      // ..
      // .. Intrinsic Functions ..
         ...
         for (CNT = 1; CNT <= ITHRESH; CNT++) {
          ...
          //     op(A) = A, A**T, or A**H depending on TRANS (and type).
            ...
            if ( Y_PREC_STATE == BASE_RESIDUAL ) {
               zhemv(UPLO, N, DCMPLX(-1.0), A, LDA, Y( 1, J ), 1, DCMPLX(1.0), RES, 1 );
            } else if ( Y_PREC_STATE == EXTRA_RESIDUAL ) {
               blas_zhemv_x(UPLO2, N, DCMPLX(-1.0), A, LDA, Y( 1, J ), 1, DCMPLX(1.0), RES, 1, PREC_TYPE);
            } else {
               blas_zhemv2_x(UPLO2, N, DCMPLX(-1.0), A, LDA, Y(1, J), Y_TAIL, 1, DCMPLX(1.0), RES, 1, PREC_TYPE);
            }
          ...
          // Check termination criteria.
            ...
            if ( Y_PREC_STATE < EXTRA_Y ) {
               zaxpy(N, DCMPLX(1.0), DY, 1, Y(1,J), 1 );
            } else {
      ...
      // Compute abs(op(A_s))*abs(Y) + abs(B_s).
         ...
         zla_heamv(UPLO2, N, 1.0, A, LDA, Y(1, J), 1, 1.0, AYB, 1 );
