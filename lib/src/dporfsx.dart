...
import 'package:lapack/src/xerbla.dart';
      ...
      void dporfsx(UPLO, EQUED, N, NRHS, final Matrix<double> A, final int LDA, final Matrix<double> AF, final int LDAF, S, final Matrix<double> B, final int LDB, final Matrix<double> X, final int LDX, RCOND, BERR, N_ERR_BNDS, ERR_BNDS_NORM, ERR_BNDS_COMP, NPARAMS, PARAMS, WORK, final Array<int> IWORK, final Box<int> INFO ) {
...
// ==================================================================
      ...
      double             DLAMCH, DLANSY, DLA_PORCOND;
      bool               lsame;
      int                ILAPREC;
      ...
      if ( REF_TYPE != 0 ) {
         ...
         PREC_TYPE = ILAPREC( 'E' );
          dla_porfsx_extended(PREC_TYPE, UPLO,  N, NRHS, A, LDA, AF, LDAF, RCEQU, S, B, LDB, X, LDX, BERR, N_NORMS, ERR_BNDS_NORM, ERR_BNDS_COMP, WORK( N+1 ), WORK( 1 ), WORK( 2*N+1 ), WORK( 1 ), RCOND, ITHRESH, RTHRESH, UNSTABLE_THRESH, IGNORE_CWISE, INFO );
      }
      ...
      // Compute scaled normwise condition number cond(A*C).
         ...
         if ( RCEQU ) {
            RCOND_TMP = DLA_PORCOND( UPLO, N, A, LDA, AF, LDAF, -1, S, INFO, WORK, IWORK );
         } else {
            RCOND_TMP = DLA_PORCOND( UPLO, N, A, LDA, AF, LDAF, 0, S, INFO, WORK, IWORK );
         }
         for (J = 1; J <= NRHS; J++) {
      ...
      // cwise error is at least CWISE_WRONG.
         ...
         for (J = 1; J <= NRHS; J++) {
            if (ERR_BNDS_COMP( J, LA_LINRX_ERR_I ) < CWISE_WRONG ) {
               RCOND_TMP = DLA_PORCOND( UPLO, N, A, LDA, AF, LDAF, 1, X( 1, J ), INFO, WORK, IWORK );
            } else {
               RCOND_TMP = 0.0;
            }
