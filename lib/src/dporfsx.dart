...
// -- Univ. of California Berkeley, Univ. of Colorado Denver and NAG Ltd..--
  ...
  if (N_ERR_BNDS >= 1 && N_NORMS >= 1) {
    ...
    if (RCEQU) {
      RCOND_TMP =
          dla_porcond(UPLO, N, A, LDA, AF, LDAF, -1, S, INFO, WORK, IWORK);
    } else {
      RCOND_TMP =
          dla_porcond(UPLO, N, A, LDA, AF, LDAF, 0, S, INFO, WORK, IWORK);
    }
    for (J = 1; J <= NRHS; J++) {
  ...
  if (N_ERR_BNDS >= 1 && N_NORMS >= 2) {
    ...
    for (J = 1; J <= NRHS; J++) {
      if (ERR_BNDS_COMP[J][LA_LINRX_ERR_I] < CWISE_WRONG) {
        RCOND_TMP = dla_porcond(
            UPLO, N, A, LDA, AF, LDAF, 1, X(1, J).asArray(), INFO, WORK, IWORK);
      } else {
        RCOND_TMP = 0.0;
      }
