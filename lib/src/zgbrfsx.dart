...
// -- Univ. of California Berkeley, Univ. of Colorado Denver and NAG Ltd..--
  ...
  if (N_ERR_BNDS >= 1 && N_NORMS >= 1) {
    ...
    if (COLEQU && NOTRAN) {
      RCOND_TMP = zla_gbrcond_c(TRANS, N, KL, KU, AB, LDAB, AFB, LDAFB, IPIV, C,
          true, INFO, WORK, RWORK);
    } else if (ROWEQU && !NOTRAN) {
      RCOND_TMP = zla_gbrcond_c(TRANS, N, KL, KU, AB, LDAB, AFB, LDAFB, IPIV, R,
          true, INFO, WORK, RWORK);
    } else {
      RCOND_TMP = zla_gbrcond_c(TRANS, N, KL, KU, AB, LDAB, AFB, LDAFB, IPIV, C,
  ...
  if (N_ERR_BNDS >= 1 && N_NORMS >= 2) {
    ...
    for (J = 1; J <= NRHS; J++) {
      if (ERR_BNDS_COMP[J][LA_LINRX_ERR_I] < CWISE_WRONG) {
        RCOND_TMP = zla_gbrcond_x(TRANS, N, KL, KU, AB, LDAB, AFB, LDAFB, IPIV,
