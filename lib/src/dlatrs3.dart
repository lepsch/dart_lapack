...
import 'package:lapack/src/xerbla.dart';
      ...
      void dlatrs3(final int UPLO, final int TRANS, final int DIAG, final int NORMIN, final int N, final int NRHS, final Matrix<double> A_, final int LDA, final Matrix<double> X_, final int LDX, final int SCALE, final int CNORM, final Array<double> WORK_, final int LWORK, final Box<int> INFO,) {
  final A = A_.dim();
  final X = X_.dim();
  final WORK = WORK_.dim();
      ...
      if ( NRHS < NRHSMIN ) {
         dlatrs(UPLO, TRANS, DIAG, NORMIN, N, A, LDA, X( 1, 1), SCALE( 1 ), CNORM, INFO );
         for (K = 2; K <= NRHS; K++) {
            dlatrs(UPLO, TRANS, DIAG, 'Y', N, A, LDA, X( 1, K ), SCALE( K ), CNORM, INFO );
         }
         return;
      }
      ...
      if ( !TMAX <= dlamch('Overflow') ) {
         ...
         for (K = 1; K <= NRHS; K++) {
            dlatrs(UPLO, TRANS, DIAG, 'N', N, A, LDA, X( 1, K ), SCALE( K ), CNORM, INFO );
         }
         return;
      }
      ...
      for (K = 1; K <= NBX; K++) {
         ...
         for (J = JFIRST; JINC < 0 ? J >= JLAST : J <= JLAST; J += JINC) {
            ...
            for (KK = 1; KK <= K2-K1; KK++) {
               RHS = K1 + KK - 1;
               if ( KK == 1 ) {
                  dlatrs(UPLO, TRANS, DIAG, 'N', J2-J1, A( J1, J1 ), LDA, X( J1, RHS ), SCALOC, CNORM, INFO );
               } else {
                  dlatrs(UPLO, TRANS, DIAG, 'Y', J2-J1, A( J1, J1 ), LDA, X( J1, RHS ), SCALOC, CNORM, INFO );
               }
            ...
            for (I = IFIRST; IINC < 0 ? I >= ILAST : I <= ILAST; I += IINC) {
               ...
               if ( NOTRAN ) {
                  ...
                  dgemm('N', 'N', I2-I1, K2-K1, J2-J1, -ONE, A( I1, J1 ), LDA, X( J1, K1 ), LDX, ONE, X( I1, K1 ), LDX );
               } else {

                  // B( I, K ) := B( I, K ) - A( J, I )**T * X( J, K )

                  dgemm('T', 'N', I2-I1, K2-K1, J2-J1, -ONE, A( J1, I1 ), LDA, X( J1, K1 ), LDX, ONE, X( I1, K1 ), LDX );
               }
            }
         }
         ...
         for (KK = 1; KK <= K2-K1; KK++) {
            ...
            if ( SCALE( RHS ) != ONE && SCALE( RHS ) != ZERO ) {
               for (I = 1; I <= NBA; I++) {
                  ...
                  if (SCAL != ONE) dscal( I2-I1, SCAL, X( I1, RHS ), 1 );
               }
            }
         }
      }
