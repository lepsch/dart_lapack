      void zla_syrfsx_extended(PREC_TYPE, UPLO, N, NRHS, final Matrix<double> A, final int LDA, final Matrix<double> AF, final int LDAF, IPIV, COLEQU, C, final Matrix<double> B, final int LDB, final Matrix<double> Y, final int LDY, BERR_OUT, N_NORMS, ERR_BNDS_NORM, ERR_BNDS_COMP, RES, AYB, DY, Y_TAIL, RCOND, ITHRESH, RTHRESH, DZ_UB, IGNORE_CWISE, final Box<int> INFO ) {
...
// =====================================================================
         ...
         for (CNT = 1; CNT <= ITHRESH; CNT++) {
          ...
          //     op(A) = A, A**T, or A**H depending on TRANS (and type).
            ...
            if ( Y_PREC_STATE == BASE_RESIDUAL ) {
               zsymv(UPLO, N, DCMPLX(-1.0), A, LDA, Y(1,J), 1, DCMPLX(1.0), RES, 1 );
            } else if ( Y_PREC_STATE == EXTRA_RESIDUAL ) {
               blas_zsymv_x(UPLO2, N, DCMPLX(-1.0), A, LDA, Y( 1, J ), 1, DCMPLX(1.0), RES, 1, PREC_TYPE );
            } else {
               blas_zsymv2_x(UPLO2, N, DCMPLX(-1.0), A, LDA, Y(1, J), Y_TAIL, 1, DCMPLX(1.0), RES, 1, PREC_TYPE);
            }

          // XXX: RES is no longer needed.
            zcopy(N, RES, 1, DY, 1 );
            zsytrs(UPLO, N, 1, AF, LDAF, IPIV, DY, N, INFO );
      ...
      // Compute abs(op(A_s))*abs(Y) + abs(B_s).
         ...
         zla_syamv(UPLO2, N, 1.0, A, LDA, Y(1, J), 1, 1.0, AYB, 1 );
