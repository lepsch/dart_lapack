      void zporfs(final int UPLO, final int N, final int NRHS, final Matrix<double> A, final int LDA, final Matrix<double> AF, final int LDAF, final Matrix<double> B, final int LDB, final Matrix<double> X, final int LDX, final int FERR, final int BERR, final Array<double> _WORK, final Array<double> RWORK, final Box<int> INFO ) {
      ...
      for (J = 1; J <= NRHS; J++) { // 140
         ...
         if ( UPPER ) {
            for (K = 1; K <= N; K++) { // 50
               ...
               RWORK[K] = RWORK( K ) + ABS( (A( K, K )).toDouble() )*XK + S;
            } // 50
         } else {
            for (K = 1; K <= N; K++) { // 70
               S = ZERO;
               XK = CABS1( X( K, J ) );
               RWORK[K] = RWORK( K ) + ABS( (A( K, K )).toDouble() )*XK;
               for (I = K + 1; I <= N; I++) { // 60
                  RWORK[I] = RWORK( I ) + CABS1( A( I, K ) )*XK;
                  S = S + CABS1( A( I, K ) )*CABS1( X( I, J ) );
               } // 60
