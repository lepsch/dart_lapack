      void zhbtrd(final int VECT, final int UPLO, final int N, final int KD, final Matrix<double> AB_, final int LDAB, final int D, final int E, final Matrix<double> Q_, final int LDQ, final Array<double> _WORK_, final Box<int> INFO,) {
  final AB = AB_.dim();
  final Q = Q_.dim();
         ...
         if ( KD > 1 ) {
            ...
            for (I = 1; I <= N - 2; I++) { // 90
               ...
               for (K = KDN + 1; K >= 2; K--) { // 80
                  ...
                  if (NR > 0) zlar2v( NR, AB( KD1, J1-1 ), AB( KD1, J1 ), AB( KD, J1 ), INCA, D( J1 ), WORK( J1 ), KD1 );
                  ...
                  if ( NR > 0 ) {
                     ...
                     // ZLARTV or ZROT is used
                        ...
                        for (L = 1; L <= KD - 1; L++) { // 30
                           ...
                           if (NRT > 0) zlartv( NRT, AB( KD-L, J1+L ), INCA, AB( KD-L+1, J1+L ), INCA, D( J1 ), WORK( J1 ), KD1 );
                        } // 30
                     } else {
                        J1END = J1 + KD1*( NR-2 );
                        if ( J1END >= J1 ) {
                        ...
                        LEND = min( KDM1, N-J2 );
                        LAST = J1END + KD1;
                        if (LEND > 0) zrot( LEND, AB( KD-1, LAST+1 ), INCX, AB( KD, LAST+1 ), INCX, D( LAST ), WORK( LAST ) );
                     }
                  }
         ...
         if ( KD > 1 ) {
            ...
            for (I = 1; I <= N - 2; I++) { // 210
               ...
               for (K = KDN + 1; K >= 2; K--) { // 200
                  ...
                  if (NR > 0) zlar2v( NR, AB( 1, J1-1 ), AB( 1, J1 ), AB( 2, J1-1 ), INCA, D( J1 ), WORK( J1 ), KD1 );
                  ...
                  if ( NR > 0 ) {
                     ...
                     if ( NR > 2*KD-1 ) {
                        for (L = 1; L <= KD - 1; L++) { // 150
                           ...
                           if (NRT > 0) zlartv( NRT, AB( L+2, J1-1 ), INCA, AB( L+1, J1 ), INCA, D( J1 ), WORK( J1 ), KD1 );
                        } // 150
                     } else {
                        J1END = J1 + KD1*( NR-2 );
                        if ( J1END >= J1 ) {
                        ...
                        LEND = min( KDM1, N-J2 );
                        LAST = J1END + KD1;
                        if (LEND > 0) zrot( LEND, AB( 3, LAST-1 ), 1, AB( 2, LAST ), 1, D( LAST ), WORK( LAST ) );
                     }
                  }
