...
import 'package:lapack/src/xerbla.dart';
      ...
      void dla_gbrfsx_extended(final int PREC_TYPE, final int TRANS_TYPE, final int N, final int KL, final int KU, final int NRHS, final Matrix<double> AB_, final int LDAB, final Matrix<double> AFB_, final int LDAFB, final Array<int> IPIV_, final int COLEQU, final int C, final Matrix<double> B_, final int LDB, final Matrix<double> Y_, final int LDY, final int BERR_OUT, final int N_NORMS, final int ERR_BNDS_NORM, final int ERR_BNDS_COMP, final int RES, final int AYB, final int DY, final int Y_TAIL, final int RCOND, final int ITHRESH, final int RTHRESH, final int DZ_UB, final int IGNORE_CWISE, final Box<int> INFO,) {
  final AB = AB_.dim();
  final AFB = AFB_.dim();
...
// -- Univ. of California Berkeley, Univ. of Colorado Denver and NAG Ltd..--
      ...
      int                IPIV( * );
      double             AB( LDAB, * ), AFB( LDAFB, * ), B( LDB, * ), Y( LDY, * ), RES(*), DY(*), Y_TAIL(*);;
...
// =====================================================================
      ...
      for (J = 1; J <= NRHS; J++) {
         ...
         //     op(A) = A, A**T, or A**H depending on TRANS (and type).
            ...
            if ( Y_PREC_STATE == BASE_RESIDUAL ) {
               dgbmv(TRANS, M, N, KL, KU, -1.0, AB, LDAB, Y( 1, J ), 1, 1.0, RES, 1 );
            } else if ( Y_PREC_STATE == EXTRA_RESIDUAL ) {
               blas_dgbmv_x(TRANS_TYPE, N, N, KL, KU, -1.0, AB, LDAB, Y( 1, J ), 1, 1.0, RES, 1, PREC_TYPE );
            } else {
               blas_dgbmv2_x(TRANS_TYPE, N, N, KL, KU, -1.0, AB, LDAB, Y( 1, J ), Y_TAIL, 1, 1.0, RES, 1, PREC_TYPE );
            }
      ...
      // or vector Z.
         ...
         dcopy(N, B( 1, J ), 1, RES, 1 );
         dgbmv(TRANS, N, N, KL, KU, -1.0, AB, LDAB, Y(1,J), 1, 1.0, RES, 1 );
      ...
      // Compute abs(op(A_s))*abs(Y) + abs(B_s).
        ...
        dla_gbamv(TRANS_TYPE, N, N, KL, KU, 1.0, AB, LDAB, Y(1, J), 1, 1.0, AYB, 1 );
