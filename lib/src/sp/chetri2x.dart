      void chetri2x(final int UPLO, final int N, final Matrix<double> A_, final int LDA, final Array<int> IPIV_, final Array<double> _WORK_, final int NB, final Box<int> INFO,) {
  final A = A_.dim();
         ...
         // Apply PERMUTATIONS P and P**H: P * inv(U**H)*inv(D)*inv(U) *P**H
            ...
            while (I <= N) {
               if ( IPIV(I) > 0 ) {
                 ...
                 IP=-IPIV(I);
                 I=I+1;
                 if ( (I-1) < IP) cheswapr( UPLO, N, A, LDA, I-1 ,IP );
                 IF ( (I-1) > IP) cheswapr( UPLO, N, A, LDA, IP ,I-1 );
              }
               I=I+1;
            }
      } else {
