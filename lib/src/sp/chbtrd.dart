...
// [LICENSE file](https://github.com/lepsch/dart_lapack/blob/main/LICENSE).
      ...
      void chbtrd(final int VECT, final int UPLO, final int N, final int KD, final Matrix<double> AB_, final int LDAB, final int D, final int E, final Matrix<double> Q_, final int LDQ, final Array<double> _WORK_, final Box<int> INFO,) {
  final AB = AB_.dim();
  final Q = Q_.dim();
            ...
            for (I = 1; I <= N - 2; I++) { // 90
               ...
               for (K = KDN + 1; K >= 2; K--) { // 80
                  ...
                  if ( NR > 0 ) {
                     ...
                     clargv(NR, AB( 1, J1-1 ), INCA, WORK( J1 ), KD1, D( J1 ), KD1 );
                     ...
                     if ( NR >= 2*KD-1 ) {
                        for (L = 1; L <= KD - 1; L++) { // 10
                           clartv(NR, AB( L+1, J1-1 ), INCA, AB( L, J1 ), INCA, D( J1 ), WORK( J1 ), KD1 );
                        } // 10

                     } else {
                        JEND = J1 + ( NR-1 )*KD1;
                        for (JINC = J1; KD1 < 0 ? JINC >= JEND : JINC <= JEND; JINC += KD1) { // 20
                           crot(KDM1, AB( 2, JINC-1 ), 1, AB( 1, JINC ), 1, D( JINC ), WORK( JINC ) );
                        } // 20
                     }
                  }
                  ...
                  if ( K > 2 ) {
                     if ( K <= N-I+1 ) {
                        ...
                        crot(K-3, AB( KD-K+4, I+K-2 ), 1, AB( KD-K+3, I+K-1 ), 1, D( I+K-1 ), WORK( I+K-1 ) );
                     }
                     NR = NR + 1;
                     J1 = J1 - KDN - 1;
                  }
                  ...
                  if (NR > 0) clar2v( NR, AB( KD1, J1-1 ), AB( KD1, J1 ), AB( KD, J1 ), INCA, D( J1 ), WORK( J1 ), KD1 );
                  ...
                  if ( NR > 0 ) {
                     ...
                     // CLARTV or CROT is used
                        ...
                        for (L = 1; L <= KD - 1; L++) { // 30
                           ...
                           if (NRT > 0) clartv( NRT, AB( KD-L, J1+L ), INCA, AB( KD-L+1, J1+L ), INCA, D( J1 ), WORK( J1 ), KD1 );
                        } // 30
                     } else {
                        J1END = J1 + KD1*( NR-2 );
                        if ( J1END >= J1 ) {
                           for (JIN = J1; KD1 < 0 ? JIN >= J1END : JIN <= J1END; JIN += KD1) { // 40
                              crot(KD-1, AB( KD-1, JIN+1 ), INCX, AB( KD, JIN+1 ), INCX, D( JIN ), WORK( JIN ) );
                           } // 40
                        }
                        LEND = min( KDM1, N-J2 );
                        LAST = J1END + KD1;
                        if (LEND > 0) crot( LEND, AB( KD-1, LAST+1 ), INCX, AB( KD, LAST+1 ), INCX, D( LAST ), WORK( LAST ) );
                     }
                  }
                  ...
                  // initially the Identity matrix
                        ...
                        for (J = J1; KD1 < 0 ? J >= J2 : J <= J2; J += KD1) { // 50
                           ...
                           crot(NQ, Q( IQB, J-1 ), 1, Q( IQB, J ), 1, D( J ), CONJG( WORK( J ) ) );
                        } // 50
                     } else {

                        for (J = J1; KD1 < 0 ? J >= J2 : J <= J2; J += KD1) { // 60
                           crot(N, Q( 1, J-1 ), 1, Q( 1, J ), 1, D( J ), CONJG( WORK( J ) ) );
                        } // 60
                     }
            ...
            for (I = 1; I <= N - 2; I++) { // 210
               ...
               for (K = KDN + 1; K >= 2; K--) { // 200
                  ...
                  if ( NR > 0 ) {
                     ...
                     clargv(NR, AB( KD1, J1-KD1 ), INCA, WORK( J1 ), KD1, D( J1 ), KD1 );
                     ...
                     if ( NR > 2*KD-1 ) {
                        for (L = 1; L <= KD - 1; L++) { // 130
                           clartv(NR, AB( KD1-L, J1-KD1+L ), INCA, AB( KD1-L+1, J1-KD1+L ), INCA, D( J1 ), WORK( J1 ), KD1 );
                        } // 130
                     } else {
                        JEND = J1 + KD1*( NR-1 );
                        for (JINC = J1; KD1 < 0 ? JINC >= JEND : JINC <= JEND; JINC += KD1) { // 140
                           crot(KDM1, AB( KD, JINC-KD ), INCX, AB( KD1, JINC-KD ), INCX, D( JINC ), WORK( JINC ) );
                        } // 140
                     }
                  ...
                  if ( K > 2 ) {
                     if ( K <= N-I+1 ) {
                        ...
                        crot(K-3, AB( K-2, I+1 ), LDAB-1, AB( K-1, I+1 ), LDAB-1, D( I+K-1 ), WORK( I+K-1 ) );
                     }
                     NR = NR + 1;
                     J1 = J1 - KDN - 1;
                  }
                  ...
                  if (NR > 0) clar2v( NR, AB( 1, J1-1 ), AB( 1, J1 ), AB( 2, J1-1 ), INCA, D( J1 ), WORK( J1 ), KD1 );
                  ...
                  if ( NR > 0 ) {
                     ...
                     if ( NR > 2*KD-1 ) {
                        for (L = 1; L <= KD - 1; L++) { // 150
                           ...
                           if (NRT > 0) clartv( NRT, AB( L+2, J1-1 ), INCA, AB( L+1, J1 ), INCA, D( J1 ), WORK( J1 ), KD1 );
                        } // 150
                     } else {
                        J1END = J1 + KD1*( NR-2 );
                        if ( J1END >= J1 ) {
                           for (J1INC = J1; KD1 < 0 ? J1INC >= J1END : J1INC <= J1END; J1INC += KD1) { // 160
                              crot(KDM1, AB( 3, J1INC-1 ), 1, AB( 2, J1INC ), 1, D( J1INC ), WORK( J1INC ) );
                           } // 160
                        }
                        LEND = min( KDM1, N-J2 );
                        LAST = J1END + KD1;
                        if (LEND > 0) crot( LEND, AB( 3, LAST-1 ), 1, AB( 2, LAST ), 1, D( LAST ), WORK( LAST ) );
                     }
                  }
                  ...
                  // initially the Identity matrix
                        ...
                        for (J = J1; KD1 < 0 ? J >= J2 : J <= J2; J += KD1) { // 170
                           ...
                           NQ = 1 + IQAEND - IQB;
                           IQAEND = min( IQAEND+KD, IQEND );
                           crot(NQ, Q( IQB, J-1 ), 1, Q( IQB, J ), 1, D( J ), WORK( J ) );
                        } // 170
                     } else {

                        for (J = J1; KD1 < 0 ? J >= J2 : J <= J2; J += KD1) { // 180
                           crot(N, Q( 1, J-1 ), 1, Q( 1, J ), 1, D( J ), WORK( J ) );
                        } // 180
                     }
                  }
