      void cgesvd(final int JOBU, final int JOBVT, final int M, final int N, final Matrix<double> A_, final int LDA, final int S, final Matrix<double> U_, final int LDU, final Matrix<double> VT_, final int LDVT, final Array<double> WORK_, final int LWORK, final Array<double> RWORK_, final Box<int> INFO,) {
  final A = A_.dim();
  final U = U_.dim();
  final VT = VT_.dim();
...
// -- Univ. of California Berkeley, Univ. of Colorado Denver and NAG Ltd..--
      ...
      if ( M >= N ) {
         ...
         if ( M >= MNTHR ) {
            ...
            if ( WNTUN ) {
               ...
               if (WNTVAS) clacpy( 'F', N, N, A, LDA, VT, LDVT );
               ...
               if ( LWORK >= N*N+3*N ) {
                  ...
                  if (N > 1) claset( 'L', N-1, N-1, CZERO, CZERO, VT( 2, 1 ), LDVT );
         ...
         if ( N >= MNTHR ) {
            ...
            if ( WNTVN ) {
               ...
               if (WNTUAS) clacpy( 'F', M, M, A, LDA, U, LDU );
      ...
      if ( ISCL == 1 ) {
         ...
         if( INFO != 0 && ANRM > BIGNUM ) slascl( 'G', 0, 0, BIGNUM, ANRM, MINMN-1, 1, RWORK( IE ), MINMN, IERR );
         if( ANRM < SMLNUM ) slascl( 'G', 0, 0, SMLNUM, ANRM, MINMN, 1, S, MINMN, IERR );
         IF( INFO != 0 && ANRM < SMLNUM ) slascl( 'G', 0, 0, SMLNUM, ANRM, MINMN-1, 1, RWORK( IE ), MINMN, IERR );
      }
