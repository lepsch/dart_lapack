      void sbdsdc(UPLO, COMPQ, N, D, E, final Matrix<double> U, final int LDU, final Matrix<double> VT, final int LDVT, Q, IQ, final Array<double> _WORK, final Array<int> IWORK, final Box<int> INFO ) {
...
// (N-1).  Sven, 17 Feb 05.
      ...
      if ( N <= SMLSIZ ) {
         if ( ICOMPQ == 2 ) {
            ...
            IU = 1;
            IVT = IU + N;
            slaset('A', N, N, ZERO, ONE, Q( IU+( QSTART-1 )*N ), N );
            slaset('A', N, N, ZERO, ONE, Q( IVT+( QSTART-1 )*N ), N );
            slasdq('U', 0, N, N, N, 0, D, E, Q( IVT+( QSTART-1 )*N ), N, Q( IU+( QSTART-1 )*N ), N, Q( IU+( QSTART-1 )*N ), N, WORK( WSTART ), INFO );
         }
         GO TO 40;
      }
