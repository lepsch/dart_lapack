      void cla_herfsx_extended(PREC_TYPE, UPLO, N, NRHS, final Matrix<double> A, final int LDA, final Matrix<double> AF, final int LDAF, IPIV, COLEQU, C, final Matrix<double> B, final int LDB, final Matrix<double> Y, final int LDY, BERR_OUT, N_NORMS, ERR_BNDS_NORM, ERR_BNDS_COMP, RES, AYB, DY, Y_TAIL, RCOND, ITHRESH, RTHRESH, DZ_UB, IGNORE_CWISE, final Box<int> INFO ) {
...
// =====================================================================
      ...
      // EXTERNAL CAXPY, CCOPY, CHETRS, CHEMV, BLAS_CHEMV_X, BLAS_CHEMV2_X, CLA_HEAMV, CLA_WWADDW, CLA_LIN_BERR
      double               SLAMCH;
      // ..
      // .. Intrinsic Functions ..
      ...
      if ( INFO != 0 ) {
         xerbla('CLA_HERFSX_EXTENDED', -INFO );
         return;
      }
      EPS = SLAMCH( 'Epsilon' );
      ...
      for (J = 1; J <= NRHS; J++) {
         ...
         for (CNT = 1; CNT <= ITHRESH; CNT++) {
          ...
          // XXX: RES is no longer needed.
            ccopy(N, RES, 1, DY, 1 );
            chetrs(UPLO, N, 1, AF, LDAF, IPIV, DY, N, INFO );
