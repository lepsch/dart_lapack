      void strevc3(SIDE, HOWMNY, SELECT, N, final Matrix<double> T, final int LDT, final Matrix<double> VL, final int LDVL, final Matrix<double> VR, final int LDVR, MM, M, final Array<double> WORK, final int LWORK, final Box<int> INFO ) {
         ...
         for (KI = N; KI >= 1; KI--) { // 140
            ...
            if ( IP == 0 ) {
               ...
               if ( !OVER ) {
                  ...
                  if (KI > 1) sgemv( 'N', N, KI-1, ONE, VR, LDVR, WORK( 1 + IV*N ), 1, WORK( KI + IV*N ), VR( 1, KI ), 1 );
         ...
         for (KI = 1; KI <= N; KI++) { // 260
            ...
            if ( IP == 0 ) {
               ...
               if ( !OVER ) {
                  ...
                  // ------------------------------
                  // version 1: back-transform each vector with GEMV, Q*x.
                  if (KI < N) sgemv( 'N', N, N-KI, ONE, VL( 1, KI+1 ), LDVL, WORK( KI+1 + IV*N ), 1, WORK( KI   + IV*N ), VL( 1, KI ), 1 );
