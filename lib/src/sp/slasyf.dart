...
// [LICENSE file](https://github.com/lepsch/dart_lapack/blob/main/LICENSE).
      ...
      void slasyf(final int UPLO, final int N, final int NB, final int KB, final Matrix<double> A_, final int LDA, final Array<int> IPIV_, final Matrix<double> W_, final int LDW, final Box<int> INFO,) {
  final A = A_.dim();
      ...
      if ( lsame( UPLO, 'U' ) ) {
         ...
         if ( max( ABSAKK, COLMAX ) == ZERO ) {
            ...
            if ( ABSAKK >= ALPHA*COLMAX ) {
               ...
               scopy(K-IMAX, A( IMAX, IMAX+1 ), LDA, W( IMAX+1, KW-1 ), 1 )                IF( K < N ) CALL SGEMV( 'No transpose', K, N-K, -ONE, A( 1, K+1 ), LDA, W( IMAX, KW+1 ), LDW, ONE, W( 1, KW-1 ), 1 );
            ...
            if ( KP != KK ) {
               ...
               A[KP][KP] = A( KK, KK );
               scopy(KK-1-KP, A( KP+1, KK ), 1, A( KP, KP+1 ), LDA )                IF( KP > 1 ) CALL SCOPY( KP-1, A( 1, KK ), 1, A( 1, KP ), 1 );
         ...
         J = K + 1;
            ...
            J = J + 1;
            if (JP != JJ && J <= N) sswap( N-J+1, A( JP, J ), LDA, A( JJ, J ), LDA );
         IF( J < N ) GO TO 60;
         ...
         if ( max( ABSAKK, COLMAX ) == ZERO ) {
            ...
            if ( ABSAKK >= ALPHA*COLMAX ) {
               ...
               scopy(N-IMAX+1, A( IMAX, IMAX ), 1, W( IMAX, K+1 ), 1 );
               sgemv('No transpose', N-K+1, K-1, -ONE, A( K, 1 ), LDA, W( IMAX, 1 ), LDW, ONE, W( K, K+1 ), 1 );
            ...
            if ( KP != KK ) {
               ...
               A[KP][KP] = A( KK, KK );
               scopy(KP-KK-1, A( KK+1, KK ), 1, A( KP, KK+1 ), LDA )                IF( KP < N ) CALL SCOPY( N-KP, A( KP+1, KK ), 1, A( KP+1, KP ), 1 );
         ...
         J = K - 1;
            ...
            // of the rows to swap back doesn't include diagonal element)
            J = J - 1;
            if (JP != JJ && J >= 1) sswap( J, A( JP, 1 ), LDA, A( JJ, 1 ), LDA );
         IF( J > 1 ) GO TO 120;
