...
// [LICENSE file](https://github.com/lepsch/dart_lapack/blob/main/LICENSE).
      ...
      void ssytrd_sb2st(final int STAGE1, final int VECT, final int UPLO, final int N, final int KD, final Matrix<double> AB_, final int LDAB, final int D, final int E, final int HOUS, final int LHOUS, final Array<double> WORK_, final int LWORK, final Box<int> INFO,) {
  final AB = AB_.dim();
...
// $OMP$     DEPEND(out:WORK(MYID))
                              ...
                              ssb2st_kernels(UPLO, WANTQ, TTYPE, STIND, EDIND, SWEEPID, N, KD, IB, WORK ( INDA ), LDA, HOUS( INDV ), HOUS( INDTAU ), LDV, WORK( INDW + TID*KD ) );
// $OMP END TASK
                          } else {
// $OMP TASK DEPEND(in:WORK(MYID+SHIFT-1))
// $OMP$     DEPEND(out:WORK(MYID))
...
// #else
                          ssb2st_kernels(UPLO, WANTQ, TTYPE, STIND, EDIND, SWEEPID, N, KD, IB, WORK ( INDA ), LDA, HOUS( INDV ), HOUS( INDTAU ), LDV, WORK( INDW ) );
// #endif
                          if ( BLKLASTIND >= (N-1) ) {
                              STT = STT + 1;
                              break;
