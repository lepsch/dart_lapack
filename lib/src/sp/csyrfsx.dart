      void csyrfsx(UPLO, EQUED, N, NRHS, final Matrix<double> A, final int LDA, final Matrix<double> AF, final int LDAF, final Array<int> IPIV, S, final Matrix<double> B, final int LDB, final Matrix<double> X, final int LDX, RCOND, BERR, N_ERR_BNDS, ERR_BNDS_NORM, ERR_BNDS_COMP, NPARAMS, PARAMS, final Array<double> _WORK, final Array<double> RWORK, final Box<int> INFO ) {
...
// -- Univ. of California Berkeley, Univ. of Colorado Denver and NAG Ltd..--
      ...
      double               RCOND;
      int                IPIV( * );
      Complex            A( LDA, * ), AF( LDAF, * ), B( LDB, * ), X( LDX, * ), WORK( * )       double               S( * ), PARAMS( * ), BERR( * ), RWORK( * ), ERR_BNDS_NORM( NRHS, * ), ERR_BNDS_COMP( NRHS, * );
      // ..
...
// ==================================================================
      ...
      double               SLAMCH, CLANSY, CLA_SYRCOND_X, CLA_SYRCOND_C;
      bool               lsame;
      int                ILAPREC;
      ...
      // cwise error is at least CWISE_WRONG.
         ...
         for (J = 1; J <= NRHS; J++) {
            if ( ERR_BNDS_COMP( J, LA_LINRX_ERR_I ) < CWISE_WRONG ) {
               RCOND_TMP = CLA_SYRCOND_X( UPLO, N, A, LDA, AF, LDAF, IPIV, X(1,J), INFO, WORK, RWORK );
            } else {
               RCOND_TMP = 0.0;
            }
