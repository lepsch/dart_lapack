      void csytri2x(final int UPLO, final int N, final Matrix<double> A_, final int LDA, final Array<int> IPIV_, final Array<double> _WORK_, final int NB, final Box<int> INFO,) {
  final A = A_.dim();
...
// Splitting Workspace
      ...
      if ( UPPER ) {
        ...
        ctrmm('L','U','T','U',NNB, NNB, ONE,A(CUT+1,CUT+1),LDA,WORK(U11+1,1),N+NB+1);
         ...
         cgemm('T','N',NNB,NNB,CUT,ONE,A(1,CUT+1),LDA, WORK,N+NB+1, ZERO, WORK(U11+1,1), N+NB+1);
       ...
       // Next Block
         ...
         // Apply PERMUTATIONS P and P**T: P * inv(U**T)*inv(D)*inv(U) *P**T
            ...
            while (I <= N) {
               if ( IPIV(I) > 0 ) {
                  IP=IPIV(I);
                 if (I < IP) csyswapr( UPLO, N, A, LDA, I ,IP );
                 if (I > IP) csyswapr( UPLO, N, A, LDA, IP ,I );
               } else {
                 IP=-IPIV(I);
                 I=I+1;
                 if ( (I-1) < IP) csyswapr( UPLO, N, A, LDA, I-1 ,IP );
                 IF ( (I-1) > IP) csyswapr( UPLO, N, A, LDA, IP ,I-1 );
              }
               I=I+1;
            }
      } else {
      ...
      // L11 Block
        ...
        ctrmm('L',UPLO,'T','U',NNB, NNB, ONE,A(CUT+1,CUT+1),LDA,WORK(U11+1,1),N+NB+1);
        ...
        if ( (CUT+NNB) < N ) {
         ...
         cgemm('T','N',NNB,NNB,N-NNB-CUT,ONE,A(CUT+NNB+1,CUT+1) ,LDA,WORK,N+NB+1, ZERO, WORK(U11+1,1), N+NB+1);
         ...
         ctrmm('L',UPLO,'T','U', N-NNB-CUT, NNB, ONE,A(CUT+NNB+1,CUT+NNB+1),LDA,WORK,N+NB+1);
