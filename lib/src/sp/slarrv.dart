...
// [LICENSE file](https://github.com/lepsch/dart_lapack/blob/main/LICENSE).
      ...
      void slarrv(final int N, final int VL, final int VU, final int D, final int L, final int PIVMIN, final int ISPLIT, final int M, final int DOL, final int DOU, final int MINRGP, final int RTOL1, final int RTOL2, final int W, final int WERR, final int WGAP, final int IBLOCK, final int INDEXW, final int GERS, final Matrix<double> Z_, final int LDZ, final int ISUPPZ, final Array<double> _WORK_, final Array<int> IWORK_, final Box<int> INFO,) {
  final Z = Z_.dim();
...
// -- Univ. of California Berkeley, Univ. of Colorado Denver and NAG Ltd..--
      ...
      ZUSEDW = ZUSEDU - ZUSEDL + 1;
       ...
       slaset('Full', N, ZUSEDW, ZERO, ZERO, Z(1,ZUSEDL), LDZ );
      ...
      for (JBLK = 1; JBLK <= IBLOCK( M ); JBLK++) { // 170
         ...
         scopy(IM, W( WBEGIN ), 1, WORK( WBEGIN ), 1 );
         ...
         if ( IDONE < IM ) {
            ...
            for (I = 1; I <= OLDNCL; I++) { // 150
               ...
               if ( NDEPTH > 0 ) {
                  ...
                  scopy(IN, Z( IBEGIN, J ), 1, D( IBEGIN ), 1 );
                  scopy(IN-1, Z( IBEGIN, J+1 ), 1, L( IBEGIN ), 1 );
                  SIGMA = Z( IEND, J+1 );

                  // Set the corresponding entries in Z to zero
                  slaset('Full', IN, 2, ZERO, ZERO, Z( IBEGIN, J), LDZ );
               }
               ...
               for (J = OLDFST; J <= OLDLST; J++) { // 140
                  ...
                  if ( NEWSIZ > 1) {
                     ...
                     slarrf(IN, D( IBEGIN ), L( IBEGIN ), WORK(INDLD+IBEGIN-1), NEWFST, NEWLST, WORK(WBEGIN), WGAP(WBEGIN), WERR(WBEGIN), SPDIAM, LGAP, RGAP, PIVMIN, TAU, Z(IBEGIN, NEWFTT),Z(IBEGIN, NEWFTT+1), WORK( INDWRK ), IINFO );
                     if ( IINFO == 0 ) {
