      void claqr5(WANTT, WANTZ, KACC22, N, KTOP, KBOT, NSHFTS, S, final Matrix<double> H, final int LDH, ILOZ, IHIZ, final Matrix<double> Z, final int LDZ, final Matrix<double> V, final int LDV, final Matrix<double> U, final int LDU, NV, final Matrix<double> WV, final int LDWV, NH, WH, final int LDWH) {
...
// ================================================================
      ...
      for (INCOL = KTOP - 2*NBMPS + 1; 2*NBMPS < 0 ? INCOL >= KBOT - 2 : INCOL <= KBOT - 2; INCOL += 2*NBMPS) { // 180
         ...
         NDCOL = INCOL + KDU;
         if (ACCUM) claset( 'ALL', KDU, KDU, ZERO, ONE, U, LDU );
