      void sgbbrd(VECT, M, N, NCC, KL, KU, final Matrix<double> AB, final int LDAB, D, E, final Matrix<double> Q, final int LDQ, final Matrix<double> PT, final int LDPT, final Matrix<double> C, final int LDC, final Array<double> _WORK, final Box<int> INFO ) {
...
// -- Univ. of California Berkeley, Univ. of Colorado Denver and NAG Ltd..--
      ...
      if (WANTQ) slaset( 'Full', M, M, ZERO, ONE, Q, LDQ );
      IF( WANTPT ) slaset( 'Full', N, N, ZERO, ONE, PT, LDPT );
      ...
      if ( KL+KU > 1 ) {
         ...
         for (I = 1; I <= MINMN; I++) { // 90
            ...
            for (KK = 1; KK <= KB; KK++) { // 80
               ...
               if (NR > 0) slargv( NR, AB( KLU1, J1-KLM-1 ), INCA, WORK( J1 ), KB1, WORK( MN+J1 ), KB1 );
               ...
               for (L = 1; L <= KB; L++) { // 10
                  ...
                  if (NRT > 0) slartv( NRT, AB( KLU1-L, J1-KLM+L-1 ), INCA, AB( KLU1-L+1, J1-KLM+L-1 ), INCA, WORK( MN+J1 ), WORK( J1 ), KB1 );
               } // 10
               ...
               if (NR > 0) slargv( NR, AB( 1, J1+KUN-1 ), INCA, WORK( J1+KUN ), KB1, WORK( MN+J1+KUN ), KB1 );
               ...
               for (L = 1; L <= KB; L++) { // 50
                  ...
                  if (NRT > 0) slartv( NRT, AB( L+1, J1+KUN-1 ), INCA, AB( L, J1+KUN ), INCA, WORK( MN+J1+KUN ), WORK( J1+KUN ), KB1 );
               } // 50
