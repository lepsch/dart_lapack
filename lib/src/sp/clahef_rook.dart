      void clahef_rook(UPLO, N, NB, KB, final Matrix<double> A, final int LDA, final Array<int> IPIV, final Matrix<double> W, final int LDW, final Box<int> INFO ) {
...
// -- Univ. of California Berkeley, Univ. of Colorado Denver and NAG Ltd..--
      ...
      double               ABSAKK, ALPHA, COLMAX, STEMP, R1, ROWMAX, T, SFMIN;
      ...
      if ( lsame( UPLO, 'U' ) ) {
         ...
         if ( max( ABSAKK, COLMAX ) == ZERO ) {
            ...
            if ( !( ABSAKK < ALPHA*COLMAX ) ) {
               ...
               DONE = false;
                  ...
                  if ( !( ABS( double( W( IMAX,KW-1 ) ) ) < ALPHA*ROWMAX ) ) {
         ...
         J = K + 1;
            ...
            J = J + 1;
            if (JP2 != JJ && J <= N) cswap( N-J+1, A( JP2, J ), LDA, A( JJ, J ), LDA );
            JJ = JJ + 1;
            if (KSTEP == 2 && JP1 != JJ && J <= N) cswap( N-J+1, A( JP1, J ), LDA, A( JJ, J ), LDA );
         IF( J < N ) GO TO 60;
         ...
         if ( max( ABSAKK, COLMAX ) == ZERO ) {
            ...
            if ( !( ABSAKK < ALPHA*COLMAX ) ) {
               ...
               // Loop until pivot found
                  ...
                  if ( !( ABS( double( W( IMAX,K+1 ) ) ) < ALPHA*ROWMAX ) ) {
         ...
         J = K - 1;
            ...
            // of the rows to swap back doesn't include diagonal element)
            J = J - 1;
            if (JP2 != JJ && J >= 1) cswap( J, A( JP2, 1 ), LDA, A( JJ, 1 ), LDA );
            JJ = JJ -1;
            if (KSTEP == 2 && JP1 != JJ && J >= 1) cswap( J, A( JP1, 1 ), LDA, A( JJ, 1 ), LDA );
         IF( J > 1 ) GO TO 120;
