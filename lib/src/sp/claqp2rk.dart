      void claqp2rk(M, N, NRHS, IOFFSET, KMAX, ABSTOL, RELTOL, KP1, MAXC2NRM, final Matrix<double> A, final int LDA, K, MAXC2NRMK, RELMAXC2NRMK, JPIV, TAU, VN1, VN2, WORK, Box<int> INFO ) {
...
// -- Univ. of California Berkeley, Univ. of Colorado Denver and NAG Ltd..--
      ...
      for (KK = 1; KK <= KMAX; KK++) {
         ...
         if ( I < M ) {
            clarfg(M-I+1, A( I, KK ), A( I+1, KK ), 1, TAU( KK ) );
         } else {
            TAU[KK] = CZERO;
         }
         ...
         if ( SISNAN( double( TAU(KK) ) ) ) {
            TAUNAN = double( TAU(KK) );
         } else if ( SISNAN( AIMAG( TAU(KK) ) ) ) {
            TAUNAN = AIMAG( TAU(KK) );
         } else {
            TAUNAN = ZERO;
         }
         ...
         if ( KK < MINMNUPDT ) {
            AIKK = A( I, KK );
            A[I][KK] = CONE;
            clarf('Left', M-I+1, N+NRHS-KK, A( I, KK ), 1, CONJG( TAU( KK ) ), A( I, KK+1 ), LDA, WORK( 1 ) );
            A[I][KK] = AIKK;
         }
