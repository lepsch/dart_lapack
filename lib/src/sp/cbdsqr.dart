      void cbdsqr(final int UPLO, final int N, final int NCVT, final int NRU, final int NCC, final int D, final int E, final Matrix<double> VT_, final int LDVT, final Matrix<double> U_, final int LDU, final Matrix<double> C_, final int LDC, final Array<double> RWORK_, final Box<int> INFO,) {
  final VT = VT_.dim();
  final U = U_.dim();
  final C = C_.dim();
...
// -- Univ. of California Berkeley, Univ. of Colorado Denver and NAG Ltd..--
      ...
      if ( LOWER ) {
         ...
         if (NRU > 0) clasr( 'R', 'V', 'F', NRU, N, RWORK( 1 ), RWORK( N ), U, LDU );
         IF( NCC > 0 ) clasr( 'L', 'V', 'F', N, NCC, RWORK( 1 ), RWORK( N ), C, LDC );
      }
      ...
      if ( LL == M-1 ) {
         ...
         if (NCVT > 0) csrot( NCVT, VT( M-1, 1 ), LDVT, VT( M, 1 ), LDVT, COSR, SINR );
         if( NRU > 0 ) csrot( NRU, U( 1, M-1 ), 1, U( 1, M ), 1, COSL, SINL );
         IF( NCC > 0 ) csrot( NCC, C( M-1, 1 ), LDC, C( M, 1 ), LDC, COSL, SINL );
         M = M - 2;
         GO TO 60;
      }
      ...
      if ( SHIFT == ZERO ) {
         if ( IDIR == 1 ) {
            ...
            if (NCVT > 0) clasr( 'L', 'V', 'F', M-LL+1, NCVT, RWORK( 1 ), RWORK( N ), VT( LL, 1 ), LDVT );
            if( NRU > 0 ) clasr( 'R', 'V', 'F', NRU, M-LL+1, RWORK( NM12+1 ), RWORK( NM13+1 ), U( 1, LL ), LDU );
            IF( NCC > 0 ) clasr( 'L', 'V', 'F', M-LL+1, NCC, RWORK( NM12+1 ), RWORK( NM13+1 ), C( LL, 1 ), LDC );
            ...
            if (NCVT > 0) clasr( 'L', 'V', 'B', M-LL+1, NCVT, RWORK( NM12+1 ), RWORK( NM13+1 ), VT( LL, 1 ), LDVT );
            if( NRU > 0 ) clasr( 'R', 'V', 'B', NRU, M-LL+1, RWORK( 1 ), RWORK( N ), U( 1, LL ), LDU );
            IF( NCC > 0 ) clasr( 'L', 'V', 'B', M-LL+1, NCC, RWORK( 1 ), RWORK( N ), C( LL, 1 ), LDC );
      ...
      for (I = 1; I <= N; I++) { // 170
         if ( D( I ) < ZERO ) {
            ...
            if (NCVT > 0) csscal( NCVT, NEGONE, VT( I, 1 ), LDVT );
         }
      } // 170
