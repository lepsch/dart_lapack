      void sgglse(final int M, final int N, final int P, final Matrix<double> A_, final int LDA, final Matrix<double> B_, final int LDB, final int C, final int D, final int X, final Array<double> WORK_, final int LWORK, final Box<int> INFO,) {
  final A = A_.dim();
  final B = B_.dim();
...
// -- Univ. of California Berkeley, Univ. of Colorado Denver and NAG Ltd..--
      ...
      if ( M < N ) {
         NR = M + P - N;
         if (NR > 0) sgemv( 'No transpose', NR, N-M, -ONE, A( N-P+1, M+1 ), LDA, D( NR+1 ), 1, ONE, C( N-P+1 ), 1 );
      } else {
         NR = P;
      }
      if ( NR > 0 ) {
