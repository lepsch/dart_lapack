...
void zlatrs3(
  ...
  if (NRHS < NRHSMIN) {
    zlatrs(UPLO, TRANS, DIAG, NORMIN, N, A, LDA, X(1, 1).asArray(), SCALE(1),
        CNORM, INFO);
    for (K = 2; K <= NRHS; K++) {
      zlatrs(UPLO, TRANS, DIAG, 'Y', N, A, LDA, X(1, K).asArray(), SCALE(K),
          CNORM, INFO);
    }
    return;
  }
  ...
  if (!(TMAX <= dlamch('Overflow'))) {
    ...
    for (K = 1; K <= NRHS; K++) {
      zlatrs(UPLO, TRANS, DIAG, 'N', N, A, LDA, X(1, K).asArray(), SCALE(K),
          CNORM, INFO);
    }
    return;
  }
  ...
  for (K = 1; K <= NBX; K++) {
    ...
    for (J = JFIRST; JINC < 0 ? J >= JLAST : J <= JLAST; J += JINC) {
      ...
      for (KK = 1; KK <= K2 - K1; KK++) {
        RHS = K1 + KK - 1;
        if (KK == 1) {
          zlatrs(UPLO, TRANS, DIAG, 'N', J2 - J1, A(J1, J1), LDA,
              X(J1, RHS).asArray(), SCALOC, CNORM, INFO);
        } else {
          zlatrs(UPLO, TRANS, DIAG, 'Y', J2 - J1, A(J1, J1), LDA,
              X(J1, RHS).asArray(), SCALOC, CNORM, INFO);
        }
        ...
        if (SCALOC.value == ZERO) {
          ...
          if (XNRM[KK] * RSCAL <= BIGNUM) {
            ...
            zdscal(J2 - J1, RSCAL, X(J1, RHS).asArray(), 1);
            SCALOC.value = ONE;
          } else {
      ...
      for (I = IFIRST; IINC < 0 ? I >= ILAST : I <= ILAST; I += IINC) {
        ...
        for (KK = 1; KK <= K2 - K1; KK++) {
          ...
          if (SCAL != ONE) {
            zdscal(I2 - I1, SCAL, X(I1, RHS).asArray(), 1);
            WORK[I + KK * LDS] = SCAMIN * SCALOC.value;
          }

          SCAL = (SCAMIN / WORK[J + KK * LDS]) * SCALOC.value;
          if (SCAL != ONE) {
            zdscal(J2 - J1, SCAL, X(J1, RHS).asArray(), 1);
        ...
        if (NOTRAN) {
          ...
          zgemm('N', 'N', I2 - I1, K2 - K1, J2 - J1, -Complex.one, A(I1, J1),
              LDA, X(J1, K1), LDX, Complex.one, X(I1, K1), LDX);
        } else if (lsame(TRANS, 'T')) {
          // B( I, K ) := B( I, K ) - A( I, J )**T * X( J, K )

          zgemm('T', 'N', I2 - I1, K2 - K1, J2 - J1, -Complex.one, A(J1, I1),
              LDA, X(J1, K1), LDX, Complex.one, X(I1, K1), LDX);
        } else {
          // B( I, K ) := B( I, K ) - A( I, J )**H * X( J, K )

          zgemm('C', 'N', I2 - I1, K2 - K1, J2 - J1, -Complex.one, A(J1, I1),
              LDA, X(J1, K1), LDX, Complex.one, X(I1, K1), LDX);
        }
      }
    }
    ...
    for (KK = 1; KK <= K2 - K1; KK++) {
      ...
      if (SCALE[RHS] != ONE && SCALE[RHS] != ZERO) {
        for (I = 1; I <= NBA; I++) {
          ...
          if (SCAL != ONE) zdscal(I2 - I1, SCAL, X(I1, RHS).asArray(), 1);
        }
      }
    }
  }
