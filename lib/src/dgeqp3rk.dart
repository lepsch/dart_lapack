...
import 'package:lapack/src/xerbla.dart';
      ...
      void dgeqp3rk(final int M, final int N, final int NRHS, final int KMAX, final int ABSTOL, final int RELTOL, final Matrix<double> A_, final int LDA, final int K, final int MAXC2NRMK, final int RELMAXC2NRMK, final int JPIV, final int TAU, final Array<double> WORK_, final int LWORK, final Array<int> IWORK_, final Box<int> INFO,) {
  ...
  final IWORK = IWORK_.dim();
      ...
      if ( NB >= NBMIN && NB < JMAX && JMAXB > 0 ) {
         ...
         while (J <= JMAXB) {
            ...
            dlaqp3rk(M, N_SUB, NRHS, IOFFSET, JB, ABSTOL, RELTOL, KP1, MAXC2NRM, A( 1, J ), LDA, DONE, JBF, MAXC2NRMK, RELMAXC2NRMK, JPIV( J ), TAU( J ), WORK( J ), WORK( N+J ), WORK( 2*N+1 ), WORK( 2*N+JB+1 ), N+NRHS-J+1, IWORK, IINFO );
      ...
      if ( J <= JMAX ) {
         ...
         dlaqp2rk(M, N_SUB, NRHS, IOFFSET, JMAX-J+1, ABSTOL, RELTOL, KP1, MAXC2NRM, A( 1, J ), LDA, KF, MAXC2NRMK, RELMAXC2NRMK, JPIV( J ), TAU( J ), WORK( J ), WORK( N+J ), WORK( 2*N+1 ), IINFO );
