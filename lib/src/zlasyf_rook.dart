      void zlasyf_rook(final int UPLO, final int N, final int NB, final int KB, final Matrix<double> A_, final int LDA, final Array<int> IPIV_, final Matrix<double> W_, final int LDW, final Box<int> INFO,) {
  final A = A_.dim();
...
// -- Univ. of California Berkeley, Univ. of Colorado Denver and NAG Ltd..--
      ...
      if ( lsame( UPLO, 'U' ) ) {
         ...
         if ( max( ABSAKK, COLMAX ) == ZERO ) {
            ...
            if ( !( ABSAKK < ALPHA*COLMAX ) ) {
               ...
               // Loop until pivot found
                  ...
                  if (K < N) zgemv( 'No transpose', K, N-K, -CONE, A( 1, K+1 ), LDA, W( IMAX, KW+1 ), LDW, CONE, W( 1, KW-1 ), 1 );
         ...
         J = K + 1;
            ...
            JJ = J - 1;
            if (JP1 != JJ && KSTEP == 2) zswap( N-J+1, A( JP1, J ), LDA, A( JJ, J ), LDA );
         IF( J <= N ) GO TO 60;
         ...
         if (K > 1) zgemv( 'No transpose', N-K+1, K-1, -CONE, A( K, 1 ), LDA, W( K, 1 ), LDW, CONE, W( K, K ), 1 );
         ...
         if ( max( ABSAKK, COLMAX ) == ZERO ) {
            ...
            if ( !( ABSAKK < ALPHA*COLMAX ) ) {
               ...
               // Loop until pivot found
                  ...
                  zcopy(N-IMAX+1, A( IMAX, IMAX ), 1, W( IMAX, K+1 ), 1 )                   IF( K > 1 ) CALL ZGEMV( 'No transpose', N-K+1, K-1, -CONE, A( K, 1 ), LDA, W( IMAX, 1 ), LDW, CONE, W( K, K+1 ), 1 );
         ...
         J = K - 1;
            ...
            JJ = J + 1;
            if (JP1 != JJ && KSTEP == 2) zswap( J, A( JP1, 1 ), LDA, A( JJ, 1 ), LDA );
         IF( J >= 1 ) GO TO 120;
