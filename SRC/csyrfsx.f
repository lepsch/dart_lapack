...
*  ==================================================================
      ...
      // cwise error is at least CWISE_WRONG.
         ...
         for (J = 1; J <= NRHS; J++) {
            IF ( ERR_BNDS_COMP( J, LA_LINRX_ERR_I ) .LT. CWISE_WRONG ) THEN                RCOND_TMP = CLA_SYRCOND_X( UPLO, N, A, LDA, AF, LDAF, IPIV, X(1,J), INFO, WORK, RWORK )
            } else {
               RCOND_TMP = 0.0
            }
      ...
      // Threshold the error (see LAWN).
            ...
            if ( RCOND_TMP .LT. ILLRCOND_THRESH ) {
               ...
               if ( .NOT. IGNORE_CWISE && INFO.LT.N + J ) INFO = N + J             ELSE IF ( ERR_BNDS_COMP( J, LA_LINRX_ERR_I ) .LT. ERR_LBND ) {
