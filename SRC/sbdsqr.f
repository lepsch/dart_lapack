...
*  =====================================================================
      ...
      if ( LOWER ) {
         ...
         if (NRU > 0) CALL SLASR( 'R', 'V', 'F', NRU, N, WORK( 1 ), WORK( N ), U, LDU )          IF( NCC > 0 ) CALL SLASR( 'L', 'V', 'F', N, NCC, WORK( 1 ), WORK( N ), C, LDC );
      }
      ...
      if ( LL == M-1 ) {
         ...
         if (NCVT > 0) CALL SROT( NCVT, VT( M-1, 1 ), LDVT, VT( M, 1 ), LDVT, COSR, SINR )          IF( NRU > 0 ) CALL SROT( NRU, U( 1, M-1 ), 1, U( 1, M ), 1, COSL, SINL )          IF( NCC > 0 ) CALL SROT( NCC, C( M-1, 1 ), LDC, C( M, 1 ), LDC, COSL, SINL );
         M = M - 2
         GO TO 60
      }
      ...
      if ( SHIFT == ZERO ) {
         if ( IDIR == 1 ) {
            ...
            if (NCVT > 0) CALL SLASR( 'L', 'V', 'F', M-LL+1, NCVT, WORK( 1 ), WORK( N ), VT( LL, 1 ), LDVT )             IF( NRU > 0 ) CALL SLASR( 'R', 'V', 'F', NRU, M-LL+1, WORK( NM12+1 ), WORK( NM13+1 ), U( 1, LL ), LDU )             IF( NCC > 0 ) CALL SLASR( 'L', 'V', 'F', M-LL+1, NCC, WORK( NM12+1 ), WORK( NM13+1 ), C( LL, 1 ), LDC );
            ...
            if (NCVT > 0) CALL SLASR( 'L', 'V', 'B', M-LL+1, NCVT, WORK( NM12+1 ), WORK( NM13+1 ), VT( LL, 1 ), LDVT )             IF( NRU > 0 ) CALL SLASR( 'R', 'V', 'B', NRU, M-LL+1, WORK( 1 ), WORK( N ), U( 1, LL ), LDU )             IF( NCC > 0 ) CALL SLASR( 'L', 'V', 'B', M-LL+1, NCC, WORK( 1 ), WORK( N ), C( LL, 1 ), LDC );
      ...
      for (I = 1; I <= N - 1; I++) { // 190
         ...
         if ( ISUB != N+1-I ) {
            ...
            if (NRU > 0) CALL SSWAP( NRU, U( 1, ISUB ), 1, U( 1, N+1-I ), 1 )             IF( NCC > 0 ) CALL SSWAP( NCC, C( ISUB, 1 ), LDC, C( N+1-I, 1 ), LDC );
         }
      } // 190
      GO TO 220
