      ...
      if ( UPPER ) {
        ...
        DO WHILE ( K .GE. 1 )
         if ( IPIV( K ).GT.0 ) {
            ...
            KP = -IPIV( K )
            IF( KP == -IPIV( K-1 ) ) CALL SSWAP( NRHS, B( K-1, 1 ), LDB, B( KP, 1 ), LDB )
            K=K-2
         }
        }
       ...
       // Compute (U**T \ B) -> B   [ U**T \ (D \ (U \P**T * B) ) ]
        ...
        DO WHILE ( K .LE. N )
         if ( IPIV( K ).GT.0 ) {
            ...
            // Interchange rows K-1 and -IPIV(K).
            KP = -IPIV( K )
            IF( K .LT. N .AND. KP == -IPIV( K+1 ) ) CALL SSWAP( NRHS, B( K, 1 ), LDB, B( KP, 1 ), LDB )
            K=K+2
         }
        }
        ...
        DO WHILE ( K .LE. N )
         if ( IPIV( K ).GT.0 ) {
            ...
            KP = -IPIV( K+1 )
            IF( KP == -IPIV( K ) ) CALL SSWAP( NRHS, B( K+1, 1 ), LDB, B( KP, 1 ), LDB )
            K=K+2
         }
        }
...
*  Compute (L**T \ B) -> B   [ L**T \ (D \ (L \P**T * B) ) ]
        ...
        DO WHILE ( K .GE. 1 )
         if ( IPIV( K ).GT.0 ) {
            ...
            // Interchange rows K-1 and -IPIV(K).
            KP = -IPIV( K )
            IF( K.GT.1 .AND. KP == -IPIV( K-1 ) ) CALL SSWAP( NRHS, B( K, 1 ), LDB, B( KP, 1 ), LDB )
            K=K-2
         }
        }
