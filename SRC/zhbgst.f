      ...
      if ( UPPER ) {
         ...
         if ( UPDATE ) {
            ...
            if ( WANTX ) {
               ...
               if (KBT > 0) CALL ZGERC( N-M, KBT, -CONE, X( M+1, I ), 1, BB( KB1-KBT, I ), 1, X( M+1, I-KBT ), LDX );
            }
         ...
         for (K = 1; K <= KB - 1; K++) { // 130
            ...
            if (NRT > 0) CALL ZLARGV( NRT, AB( 1, J2T ), INCA, WORK( J2T-M ), KA1, RWORK( J2T-M ), KA1 );
            if ( NR > 0 ) {
            ...
            DO 110 L = KA - 1, KB - K + 1, -1;
               NRT = ( N-J2+L ) / KA1;
               if (NRT > 0) CALL ZLARTV( NRT, AB( L, J2+KA1-L ), INCA, AB( L+1, J2+KA1-L ), INCA, RWORK( J2-M ), WORK( J2-M ), KA1 );
            } // 110
         ...
         DO 170 K = KB, 1, -1;
            ...
            DO 140 L = KB - K, 1, -1;
               NRT = ( N-J2+KA+L ) / KA1;
               if (NRT > 0) CALL ZLARTV( NRT, AB( L, J2-L+1 ), INCA, AB( L+1, J2-L+1 ), INCA, RWORK( J2-KA ), WORK( J2-KA ), KA1 );
            } // 140
            NR = ( N-J2+KA ) / KA1;
            J1 = J2 + ( NR-1 )*KA1;
            DO 150 J = J1, J2, -KA1;
         ...
         DO 210 K = KB, 1, -1;
            ...
            DO 190 L = KA - 1, KB - K + 1, -1;
               NRT = ( N-J2+L ) / KA1;
               if (NRT > 0) CALL ZLARTV( NRT, AB( L, J2+KA1-L ), INCA, AB( L+1, J2+KA1-L ), INCA, RWORK( J2 ), WORK( J2 ), KA1 );
            } // 190
         ...
         if ( UPDATE ) {
            ...
            if ( WANTX ) {
               ...
               zdscal(N-M, ONE / BII, X( M+1, I ), 1 );
               if (KBT > 0) CALL ZGERU( N-M, KBT, -CONE, X( M+1, I ), 1, BB( KBT+1, I-KBT ), LDBB-1, X( M+1, I-KBT ), LDX );
            }
         ...
         for (K = 1; K <= KB - 1; K++) { // 360
            ...
            if (NRT > 0) CALL ZLARGV( NRT, AB( KA1, J2T-KA ), INCA, WORK( J2T-M ), KA1, RWORK( J2T-M ), KA1 );
            if ( NR > 0 ) {
            ...
            DO 340 L = KA - 1, KB - K + 1, -1;
               NRT = ( N-J2+L ) / KA1;
               if (NRT > 0) CALL ZLARTV( NRT, AB( KA1-L+1, J2 ), INCA, AB( KA1-L, J2+1 ), INCA, RWORK( J2-M ), WORK( J2-M ), KA1 );
            } // 340
         ...
         DO 400 K = KB, 1, -1;
            ...
            DO 370 L = KB - K, 1, -1;
               NRT = ( N-J2+KA+L ) / KA1;
               if (NRT > 0) CALL ZLARTV( NRT, AB( KA1-L+1, J2-KA ), INCA, AB( KA1-L, J2-KA+1 ), INCA, RWORK( J2-KA ), WORK( J2-KA ), KA1 );
            } // 370
            NR = ( N-J2+KA ) / KA1;
            J1 = J2 + ( NR-1 )*KA1;
            DO 380 J = J1, J2, -KA1;
         ...
         DO 440 K = KB, 1, -1;
            ...
            DO 420 L = KA - 1, KB - K + 1, -1;
               NRT = ( N-J2+L ) / KA1;
               if (NRT > 0) CALL ZLARTV( NRT, AB( KA1-L+1, J2 ), INCA, AB( KA1-L, J2+1 ), INCA, RWORK( J2 ), WORK( J2 ), KA1 );
            } // 420
      ...
      if ( UPPER ) {
         ...
         if ( UPDATE ) {
            ...
            if ( WANTX ) {
               ...
               if (KBT > 0) CALL ZGERU( NX, KBT, -CONE, X( 1, I ), 1, BB( KB, I+1 ), LDBB-1, X( 1, I+1 ), LDX );
            }
         ...
         for (K = 1; K <= KB - 1; K++) { // 610
            ...
            if (NRT > 0) CALL ZLARGV( NRT, AB( 1, J1+KA ), INCA, WORK( J1 ), KA1, RWORK( J1 ), KA1 );
            if ( NR > 0 ) {
            ...
            DO 590 L = KA - 1, KB - K + 1, -1;
               NRT = ( J2+L-1 ) / KA1;
               J1T = J2 - ( NRT-1 )*KA1;
               if (NRT > 0) CALL ZLARTV( NRT, AB( L, J1T ), INCA, AB( L+1, J1T-1 ), INCA, RWORK( J1T ), WORK( J1T ), KA1 );
            } // 590
         ...
         DO 650 K = KB, 1, -1;
            ...
            DO 620 L = KB - K, 1, -1;
               NRT = ( J2+KA+L-1 ) / KA1;
               J1T = J2 - ( NRT-1 )*KA1;
               if (NRT > 0) CALL ZLARTV( NRT, AB( L, J1T+KA ), INCA, AB( L+1, J1T+KA-1 ), INCA, RWORK( M-KB+J1T+KA ), WORK( M-KB+J1T+KA ), KA1 );
            } // 620
            NR = ( J2+KA-1 ) / KA1;
            J1 = J2 - ( NR-1 )*KA1;
            DO 630 J = J1, J2, KA1;
         ...
         DO 690 K = KB, 1, -1;
            ...
            DO 670 L = KA - 1, KB - K + 1, -1;
               NRT = ( J2+L-1 ) / KA1;
               J1T = J2 - ( NRT-1 )*KA1;
               if (NRT > 0) CALL ZLARTV( NRT, AB( L, J1T ), INCA, AB( L+1, J1T-1 ), INCA, RWORK( M-KB+J1T ), WORK( M-KB+J1T ), KA1 );
            } // 670
         ...
         if ( UPDATE ) {
            ...
            if ( WANTX ) {
               ...
               zdscal(NX, ONE / BII, X( 1, I ), 1 );
               if (KBT > 0) CALL ZGERC( NX, KBT, -CONE, X( 1, I ), 1, BB( 2, I ), 1, X( 1, I+1 ), LDX );
            }
         ...
         for (K = 1; K <= KB - 1; K++) { // 840
            ...
            if (NRT > 0) CALL ZLARGV( NRT, AB( KA1, J1 ), INCA, WORK( J1 ), KA1, RWORK( J1 ), KA1 );
            if ( NR > 0 ) {
            ...
            DO 820 L = KA - 1, KB - K + 1, -1;
               NRT = ( J2+L-1 ) / KA1;
               J1T = J2 - ( NRT-1 )*KA1;
               if (NRT > 0) CALL ZLARTV( NRT, AB( KA1-L+1, J1T-KA1+L ), INCA, AB( KA1-L, J1T-KA1+L ), INCA, RWORK( J1T ), WORK( J1T ), KA1 );
            } // 820
         ...
         DO 880 K = KB, 1, -1;
            ...
            DO 850 L = KB - K, 1, -1;
               NRT = ( J2+KA+L-1 ) / KA1;
               J1T = J2 - ( NRT-1 )*KA1;
               if (NRT > 0) CALL ZLARTV( NRT, AB( KA1-L+1, J1T+L-1 ), INCA, AB( KA1-L, J1T+L-1 ), INCA, RWORK( M-KB+J1T+KA ), WORK( M-KB+J1T+KA ), KA1 );
            } // 850
            NR = ( J2+KA-1 ) / KA1;
            J1 = J2 - ( NR-1 )*KA1;
            DO 860 J = J1, J2, KA1;
         ...
         DO 920 K = KB, 1, -1;
            ...
            DO 900 L = KA - 1, KB - K + 1, -1;
               NRT = ( J2+L-1 ) / KA1;
               J1T = J2 - ( NRT-1 )*KA1;
               if (NRT > 0) CALL ZLARTV( NRT, AB( KA1-L+1, J1T-KA1+L ), INCA, AB( KA1-L, J1T-KA1+L ), INCA, RWORK( M-KB+J1T ), WORK( M-KB+J1T ), KA1 );
            } // 900
