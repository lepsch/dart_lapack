...
*  =====================================================================
      ...
      if ( LSAME( UPLO, 'U' ) ) {
         ...
         if (K < N) CALL CGEMV( 'No transpose', K, N-K, -CONE, A( 1, K+1 ), LDA, W( K, KW+1 ), LDW, CONE, W( 1, KW ), 1 );
         ...
         if ( MAX( ABSAKK, COLMAX ) == ZERO ) {
            ...
            if ( ABSAKK.GE.ALPHA*COLMAX ) {
               ...
               ccopy(K-IMAX, A( IMAX, IMAX+1 ), LDA, W( IMAX+1, KW-1 ), 1 )                IF( K < N ) CALL CGEMV( 'No transpose', K, N-K, -CONE, A( 1, K+1 ), LDA, W( IMAX, KW+1 ), LDW, CONE, W( 1, KW-1 ), 1 );
         ...
         J = K + 1
            ...
            J = J + 1
            if (JP != JJ && J.LE.N) CALL CSWAP( N-J+1, A( JP, J ), LDA, A( JJ, J ), LDA )          IF( J < N ) GO TO 60;
         ...
         if ( MAX( ABSAKK, COLMAX ) == ZERO ) {
            ...
            if ( ABSAKK.GE.ALPHA*COLMAX ) {
               ...
               ccopy(N-IMAX+1, A( IMAX, IMAX ), 1, W( IMAX, K+1 ), 1 );
         ...
         J = K - 1
            ...
            // of the rows to swap back doesn't include diagonal element)
            J = J - 1
            if (JP != JJ && J.GE.1) CALL CSWAP( J, A( JP, 1 ), LDA, A( JJ, 1 ), LDA )          IF( J.GT.1 ) GO TO 120;
