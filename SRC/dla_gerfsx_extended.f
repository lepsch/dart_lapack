...
*  =====================================================================
         ...
         for (CNT = 1; CNT <= ITHRESH; CNT++) {
          ...
          // Compute residual RES = B_s - op(A_s) * Y,
            ...
            if ( Y_PREC_STATE .EQ. BASE_RESIDUAL ) {
               dgemv(TRANS, N, N, -1.0D+0, A, LDA, Y( 1, J ), 1, 1.0D+0, RES, 1 );
            } else if ( Y_PREC_STATE .EQ. EXTRA_RESIDUAL ) {
               blas_dgemv_x(TRANS_TYPE, N, N, -1.0D+0, A, LDA, Y( 1, J ), 1, 1.0D+0, RES, 1, PREC_TYPE );
            } else {
               blas_dgemv2_x(TRANS_TYPE, N, N, -1.0D+0, A, LDA, Y( 1, J ), Y_TAIL, 1, 1.0D+0, RES, 1, PREC_TYPE );
            }
      ...
      // Compute abs(op(A_s))*abs(Y) + abs(B_s).
         ...
         dla_geamv(TRANS_TYPE, N, N, 1.0D+0, A, LDA, Y(1, J), 1, 1.0D+0, AYB, 1 );
