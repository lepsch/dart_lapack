...
*  =====================================================================
      ...
      if ( LSAME( UPLO, 'U' ) ) {
         ...
         J = K + 1
            ...
            J = J + 1
            if (JP2.NE.JJ .AND. J.LE.N) CALL SSWAP( N-J+1, A( JP2, J ), LDA, A( JJ, J ), LDA );
            JJ = J - 1
            if (JP1.NE.JJ .AND. KSTEP.EQ.2) CALL SSWAP( N-J+1, A( JP1, J ), LDA, A( JJ, J ), LDA )          IF( J.LE.N ) GO TO 60;
         ...
         if ( MAX( ABSAKK, COLMAX ).EQ.ZERO ) {
            ...
            if ( .NOT.( ABSAKK.LT.ALPHA*COLMAX ) ) {
               ...
               // Loop until pivot found
                  ...
                  scopy(N-IMAX+1, A( IMAX, IMAX ), 1, W( IMAX, K+1 ), 1 )                   IF( K.GT.1 ) CALL SGEMV( 'No transpose', N-K+1, K-1, -ONE, A( K, 1 ), LDA, W( IMAX, 1 ), LDW, ONE, W( K, K+1 ), 1 );
         ...
         J = K - 1
            ...
            J = J - 1
            if (JP2.NE.JJ .AND. J.GE.1) CALL SSWAP( J, A( JP2, 1 ), LDA, A( JJ, 1 ), LDA );
            JJ = J + 1
            if (JP1.NE.JJ .AND. KSTEP.EQ.2) CALL SSWAP( J, A( JP1, 1 ), LDA, A( JJ, 1 ), LDA )          IF( J.GE.1 ) GO TO 120;
