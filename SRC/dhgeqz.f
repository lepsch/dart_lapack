...
// $                     SAFETY = 1.0 )
      ...
      for (JITER = 1; JITER <= MAXIT; JITER++) { // 360
         ...
         if ( ( IITER / 10 )*10 == IITER ) {
            ...
            IF( ( DBLE( MAXIT )*SAFMIN )*ABS( H( ILAST, ILAST-1 ) ) < ABS( T( ILAST-1, ILAST-1 ) ) ) THEN                ESHIFT = H( ILAST, ILAST-1 ) / T( ILAST-1, ILAST-1 );
            } else {
         ...
         if ( IFIRST+1 == ILAST ) {
            ...
            if (ILAST < ILASTM) CALL DROT( ILASTM-ILAST, T( ILAST-1, ILAST+1 ), LDT, T( ILAST, ILAST+1 ), LDT, CL, SL )             IF( IFRSTM < ILAST-1 ) CALL DROT( IFIRST-IFRSTM, T( IFRSTM, ILAST-1 ), 1, T( IFRSTM, ILAST ), 1, CR, SR );
