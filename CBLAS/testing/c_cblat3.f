...
// -- Written on 8-February-1989.
      ...
      // .. Data statements ..
      const SNAMES = ['cblas_cgemm ', 'cblas_chemm ', 'cblas_csymm ', 'cblas_ctrmm ', 'cblas_ctrsm ', 'cblas_cherk ', 'cblas_csyrk ', 'cblas_cher2k', 'cblas_csyr2k'];
                  ...
                  for (ICB = 1; ICB <= 3; ICB++) { // 70
                     ...
                     for (IA = 1; IA <= NALF; IA++) { // 60
                        ...
                        for (IB = 1; IB <= NBET; IB++) { // 50
                           ...
                           if (TRACE) cprcn1(NTRA, NC, SNAME, IORDER, TRANSA, TRANSB, M, N, K, ALPHA, LDA, LDB, BETA, LDC);
                           if (REWI) REWIND NTRA;
               ...
               for (ICU = 1; ICU <= 2; ICU++) { // 70
                  ...
                  for (IA = 1; IA <= NALF; IA++) { // 60
                     ...
                     for (IB = 1; IB <= NBET; IB++) { // 50
                        ...
                        if (TRACE) cprcn2(NTRA, NC, SNAME, IORDER, SIDE, UPLO, M, N, ALPHA, LDA, LDB, BETA, LDC);
                        if (REWI) REWIND NTRA;
                        if ( CONJ ) {
                  ...
                  for (ICT = 1; ICT <= 3; ICT++) { // 100
                     ...
                     for (ICD = 1; ICD <= 2; ICD++) { // 90
                        ...
                        for (IA = 1; IA <= NALF; IA++) { // 80
                           ...
                           if ( SNAME( 10: 11 ) == 'mm' ) {
                              if (TRACE) cprcn3( NTRA, NC, SNAME, IORDER, SIDE, UPLO, TRANSA, DIAG, M, N, ALPHA, LDA, LDB);
                              if (REWI) REWIND NTRA;
               ...
               for (ICU = 1; ICU <= 2; ICU++) { // 70
                  ...
                  for (IA = 1; IA <= NALF; IA++) { // 60
                     ...
                     for (IB = 1; IB <= NBET; IB++) { // 50
                        ...
                        if ( CONJ ) {
                           if (TRACE) cprcn6( NTRA, NC, SNAME, IORDER, UPLO, TRANS, N, K, RALPHA, LDA, RBETA, LDC);
                           if (REWI) REWIND NTRA;
                           ccherk(IORDER, UPLO, TRANS, N, K, RALPHA, AA, LDA, RBETA, CC, LDC );
                        } else {
                           if (TRACE) cprcn4( NTRA, NC, SNAME, IORDER, UPLO, TRANS, N, K, ALPHA, LDA, BETA, LDC);
                           if (REWI) REWIND NTRA;
               ...
               for (ICU = 1; ICU <= 2; ICU++) { // 100
                  ...
                  for (IA = 1; IA <= NALF; IA++) { // 90
                     ...
                     for (IB = 1; IB <= NBET; IB++) { // 80
                        ...
                        if ( CONJ ) {
                           if (TRACE) cprcn7( NTRA, NC, SNAME, IORDER, UPLO, TRANS, N, K, ALPHA, LDA, LDB, RBETA, LDC);
                           if (REWI) REWIND NTRA;
                           ccher2k(IORDER, UPLO, TRANS, N, K, ALPHA, AA, LDA, BB, LDB, RBETA, CC, LDC );
                        } else {
                           if (TRACE) cprcn5( NTRA, NC, SNAME, IORDER, UPLO, TRANS, N, K, ALPHA, LDA, LDB, BETA, LDC);
                           if (REWI) REWIND NTRA;
...
// -- Written on 8-February-1989.
      ...
      void cmmch(final int TRANSA, final int TRANSB, final int M, final int N, final int KK, final int ALPHA, final Matrix<double> A, final int LDA, final Matrix<double> B, final int LDB, final int BETA, final Matrix<double> C, final int LDC, final int CT, final int G, final Matrix<double> CC_, final int LDCC, final int EPS, final int ERR, final int FATAL, final int NOUT, final int MV,) {
  final CC = CC_.dim();
