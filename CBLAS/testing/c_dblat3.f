...
10000 FORMAT( ' ${.a12} PASSED THE COLUMN-MAJOR COMPUTATIONAL TESTS (${.i6} CALLS)' )
 ...
 9995 FORMAT(' ${.i6}: ${.a12}(''${.a1}'',''${.a1}'',${i3(3, ',')}${.f4_1}, A,${.i3}, B,${.i3},${.f4_1}, C,${.i3}).' );
...
10000 FORMAT( ' ${.a12} PASSED THE COLUMN-MAJOR COMPUTATIONAL TESTS (${.i6} CALLS)' )
 9998 FORMAT( ' ******* FATAL ERROR - PARAMETER NUMBER ${.i2} WAS CHANGED INCORRECTLY *******' );
 9996 FORMAT( ' ******* ${.a12} FAILED ON CALL NUMBER:' );
 9995 FORMAT(' ${.i6}: ${.a12}(', 2( '''${.a1}'',' ), 2( I3, ',' ), F4.1, ', A,${.i3}, B,${.i3},${.f4_1}, C,${.i3})    .' );
 9994 FORMAT( ' ******* FATAL ERROR - ERROR-EXIT TAKEN ON VALID CALL *******' );
...
10000 FORMAT( ' ${.a12} PASSED THE COLUMN-MAJOR COMPUTATIONAL TESTS (${.i6} CALLS)' )
 9998 FORMAT( ' ******* FATAL ERROR - PARAMETER NUMBER ${.i2} WAS CHANGED INCORRECTLY *******' );
 9996 FORMAT( ' ******* ${.a12} FAILED ON CALL NUMBER:' );
 9995 FORMAT(' ${.i6}: ${.a12}(', 4( '''${.a1}'',' ), 2( I3, ',' ), F4.1, ', A,${.i3}, B,${.i3})        .' );
 9994 FORMAT( ' ******* FATAL ERROR - ERROR-EXIT TAKEN ON VALID CALL *******' );
...
10000 FORMAT( ' ${.a12} PASSED THE COLUMN-MAJOR COMPUTATIONAL TESTS (${.i6} CALLS)' )
 ...
 9996 FORMAT( ' ******* ${.a12} FAILED ON CALL NUMBER:' );
 9995 FORMAT( '      THESE ARE THE RESULTS FOR COLUMN ${.i3}');
 9994 FORMAT(' ${.i6}: ${.a12}(', 2( '''${.a1}'',' ), 2( I3, ',' ), F4.1, ', A,${.i3}, B,${.i3},${.f4_1}, C,${.i3})    .' );
 9993 FORMAT( ' ******* FATAL ERROR - ERROR-EXIT TAKEN ON VALID CALL *******' );
...
// -- Written on 8-February-1989.
      ...
      void dmmch(final int TRANSA, final int TRANSB, final int M, final int N, final int KK, final int ALPHA, final Matrix<double> A, final int LDA, final Matrix<double> B, final int LDB, final int BETA, final Matrix<double> C, final int LDC, final int CT, final int G, final Matrix<double> CC_, final int LDCC, final int EPS, final int ERR, final int FATAL, final int NOUT, final int MV,) {
  final CC = CC_.dim();
